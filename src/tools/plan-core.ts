...
const SUBMIT_PLAN_DESCRIPTION =
  "Submit ONE concrete plan you've already decided on. Use this for tasks that warrant a review gate ? multi-file refactors, architecture changes, anything expensive to undo. Skip it for small fixes. If /plan mode is enabled, writes are blocked at dispatch and you MUST use this. Do NOT use submit_plan for A/B/C menus; call `ask_choice` for branching decisions. STRONGLY PREFERRED: pass `steps` ? an array of {id, title, action, risk?, topology}. Every step MUST include topology: REUSE, MODIFY, or CREATE_CHILD. After each step, call `mark_step_complete`.";
