...
const STEP_ITEM_SCHEMA = {
  ...
  properties: {
    ...
    topology: {
      type: "string",
      enum: ["REUSE", "MODIFY", "CREATE_CHILD"],
      description:
        "TriadMind topology operation tag. REUSE = call existing node without changing it. MODIFY = extend existing node's interface. CREATE_CHILD = new node under an existing parent. EVERY step MUST carry one of these three tags. The UI shows the tag as a badge next to the step title.",
    },
  },
  required: ["id", "title", "action", "topology"],
};
