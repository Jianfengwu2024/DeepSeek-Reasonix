...
import { AnalyzerOptions, calculateBlastRadius, calculateDownstreamFanoutNodes, detectCycles, DownstreamFanoutNode } from './analyzer';
...
type AggregateGroup = {
    ...
    helperLikeCount: number;
    uniqueSourcePaths: Set<string>;
    averageDownstream: number;
};
...
function buildRenormalizeCandidates(nodes: TriadMapNodeLike[], options?: AnalyzerOptions): TriadizationProposal[] {
            ...
            return {
                ...
                taskBundle: [
                    createTriadizationTask(
                        'renormalize',
                        '重整化当前环拓扑',
                        `先把 ${targetNodeIds.join(', ')} 提升为同一宏观顶点，再重建其外部 demand / answer 边界。`
                    ),
                    createTriadizationTask(
                        'macro',
                        '确认新挂载点与左右分支',
                        '在环被提升后，重新确认这个宏观顶点的挂载点、左分支能力和右分支约束。'
                    ),
                    createTriadizationTask(
                        'protocol',
                        '生成环重整化协议',
                        '把新宏观顶点、被吸收节点和新的边界写入 draft-protocol.json。'
                    ),
                    createTriadizationTask(
                        'verify',
                        '验证环是否被解开',
                        '刷新 triad-map 并确认没有新的 broken contract 或残留回环。'
                    )
                ],
...
function buildSplitCandidates(
    ...
    downstreamEntries: DownstreamFanoutNode[],
    options?: AnalyzerOptions
): TriadizationProposal[] {
            ...
            return {
                ...
                taskBundle: [
                    createTriadizationTask(
                        'macro',
                        '确认当前挂载点',
                        `确认 ${entry.nodeId} 是否仍是本轮挂载点，并切出左分支子功能与右分支约束。`
                    ),
                    createTriadizationTask(
                        'meso',
                        '拆出子能力与编排件',
                        '把执行动作、策略编排和配置状态拆成更清晰的能力节点与数据管道。'
                    ),
                    createTriadizationTask(
                        'micro',
                        '标注静态右支与动态左支',
                    ...
                    createTriadizationTask(
                        'protocol',
                        '写入最小演进协议',
                        '优先生成可审阅的最小 split 协议，而不是一次性扩题。'
                    ),
                    createTriadizationTask(
                        'verify',
                        '检查扇出是否下降',
                        '刷新 triad-map 并确认切分后没有新增 drift。'
                    )
                ],
...
function buildAggregateCandidates(
    nodes: TriadMapNodeLike[],
    downstreamByNode: Map<string, DownstreamFanoutNode>
): TriadizationProposal[] {
            ...
            return {
                ...
                taskBundle: [
                    createTriadizationTask(
                        'macro',
                        '确认聚合挂载点',
                        `为 ${group.groupKey} 指定统一挂载点，并重新定义聚合后顶点的外部职责。`
                    ),
                    createTriadizationTask(
                        'meso',
                        '收束离散子节点',
                        '把同源碎叶收束为一个主顶点，再决定哪些子叶保留为左分支。'
                    ),
                    createTriadizationTask(
                        'protocol',
                        '生成聚合协议',
                        '优先使用 reuse / modify，必要时再 create_child，避免横向扩散。'
                    ),
                    createTriadizationTask(
                        'verify',
                        '验证主图可读性',
                        '确认聚合后主图顶点更少、语义更清晰，且没有丢失关键输入输出。'
                    )
                ],
    ...
    return {
        ...
        taskBundle: [
            ...
            createTriadizationTask(
                'micro',
                '补齐左右分支',
                '先把该节点的静态右支与动态左支明确出来，再决定是否继续裂变。'
            ),
            createTriadizationTask(
                'protocol',
                '形成第一版协议',
                '把这次确认后的最小演进动作写入 draft-protocol.json。'
            )
        ],
...
function toAggregateGroup(
    groupKey: string,
    groupNodes: TriadMapNodeLike[],
    downstreamByNode: Map<string, DownstreamFanoutNode>
): AggregateGroup {
...
function detectHighFanoutNodes(map: TriadMapNodeLike[], options?: AnalyzerOptions): DownstreamFanoutNode[] {
    const nodeById = new Map(map.map((node) => [getNodeId(node), node]));
    return calculateDownstreamFanoutNodes(map, 1, options).filter((entry) => {
