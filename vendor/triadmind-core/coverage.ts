...
import { readRuntimeMapArtifact, readTriadNodesArtifact } from './artifactReaders';
import { loadTriadConfig, resolveCategoryFromConfig, shouldExcludeSourcePath } from './config';
import { TriadCategory } from './protocol';
import { RuntimeEdge, RuntimeEvidence, RuntimeNode } from './runtime/types';
import { safeWalkProject } from './sourceWalker';
import { CoveragePaths, normalizePath } from './workspace';
...
export function runCoverage(paths: CoveragePaths): CoverageReport {
    ...
    const byCategoryEntries = Object.keys(config.categories).map((rawCategory) => {
        ...
        const categoryFiles = universeFiles.filter(
            (relativePath) => resolveCategoryFromConfig(relativePath, config) === category
        );
        return [
            category,
            buildBucketReport({
...
function readTriadNodes(filePath: string) {
    return readTriadNodesArtifact(filePath)
        .map((node) => String(node?.sourcePath ?? ''))
        .filter(Boolean);
}

function collectRuntimeSourcePaths(filePath: string) {
    const runtimeMap = readRuntimeMapArtifact(filePath);
    if (!runtimeMap) {
        return [] as string[];
    }
