import { readJsonArrayArtifactResult, readJsonFileStrict } from './artifactReaders';
import {
...
import { normalizeTriadizationFocusReference } from './triadizationFocusSupport';
...
export function readTriadMap(mapPath: string) {
    const result = readJsonArrayArtifactResult<unknown>(mapPath);
    if (result.status !== 'ok' || !result.value) {
        return [] as TriadNodeDefinition[];
    }

    try {
        return getTriadNodeDefinitionSchema().array().parse(result.value);
    } catch {
        return [] as TriadNodeDefinition[];
    }
}
...
export function readJsonFile<T>(filePath: string): T {
    return readJsonFileStrict<T>(filePath);
}
