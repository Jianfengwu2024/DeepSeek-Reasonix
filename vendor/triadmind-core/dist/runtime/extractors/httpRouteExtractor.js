...
exports.httpRouteExtractor = void 0;
const runtimeUtils_1 = require("../runtimeUtils");
const HTTP_METHODS = new Set(['get', 'post', 'put', 'delete', 'patch', 'options', 'head']);
exports.httpRouteExtractor = {
    ...
    detect(context) {
        ...
        return (hint === 'fastapi' ||
            ...
            hint === 'express' ||
            hint === 'nestjs' ||
            context.files.some((file) => /@(?:app|router|blueprint)\.(?:get|post|put|delete|patch|options|head|route)\(|\b(?:app|router)\.(?:get|post|put|delete|patch|options|head)\(|@(Get|Post|Put|Delete|Patch)\(/.test(file.content)));
    },
    extract(context) {
        const nodes = [];
        const edges = [];
...
function extractPythonRoutes(file, frameworkHint, nodes, edges) {
    const routeRegex = /@(?:(?:\w+\.)?(get|post|put|delete|patch|options|head)|(?:app|router|blueprint)\.route)\(\s*["']([^"']+)["']([^)]*)\)\s*(?:\r?\n\s*)+(?:async\s+def|def)\s+([A-Za-z_][\w]*)\s*\(/gi;
    for (const match of file.content.matchAll(routeRegex)) {
        ...
        const routePath = (0, runtimeUtils_1.normalizeApiPath)(match[2]);
        ...
        for (const method of methods) {
            const routeId = (0, runtimeUtils_1.apiRouteId)(method, routePath);
            nodes.push({
                id: routeId,
                type: 'ApiRoute',
                ...
                evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'decorator', decoratorText, match.index, 0.95)]
            });
            for (const call of findServiceCalls(body)) {
                const serviceId = (0, runtimeUtils_1.inferServiceId)(call.text);
                nodes.push({
                    id: serviceId,
                    type: 'Service',
                    ...
                    evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'call', call.text, (match.index ?? 0) + call.index, 0.65)]
                });
                edges.push({
                    from: routeId,
                    to: serviceId,
...
function extractExpressRoutes(file, nodes, edges) {
    ...
    for (const match of file.content.matchAll(routeRegex)) {
        ...
        nodes.push({
            ...
            evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'call', match[0], match.index, 0.9)]
        });
        for (const call of findServiceCalls(handler)) {
            ...
            nodes.push({
                ...
                sourcePath: file.relativePath,
                category: 'backend',
                evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'call', call.text, (match.index ?? 0) + call.index, 0.6)]
            });
            edges.push({
                from: routeId,
                to: serviceId,
...
function extractNestRoutes(file, nodes, edges) {
    const controllerMatch = file.content.match(/@Controller\(\s*["'`]([^"'`]*)["'`]\s*\)/);
    const prefix = controllerMatch ? (0, runtimeUtils_1.normalizeApiPath)(controllerMatch[1]) : '';
    const routeRegex = /@(Get|Post|Put|Delete|Patch)\(\s*["'`]([^"'`]*)["'`]\s*\)\s*(?:\r?\n\s*)+(?:async\s+)?([A-Za-z_][\w]*)\s*\(/g;
    for (const match of file.content.matchAll(routeRegex)) {
        const method = match[1].toUpperCase();
        const routePath = (0, runtimeUtils_1.normalizeApiPath)(`${prefix}/${match[2]}`.replace(/\/+/g, '/'));
        const routeId = (0, runtimeUtils_1.apiRouteId)(method, routePath);
        ...
        nodes.push({
            ...
            evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'decorator', match[0], match.index, 0.9)]
        });
        for (const call of findServiceCalls(body)) {
            const serviceId = (0, runtimeUtils_1.inferServiceId)(call.text);
            nodes.push({
...
function resolvePythonMethods(decoratorMethod, methodsText) {
    if (decoratorMethod && HTTP_METHODS.has(decoratorMethod.toLowerCase())) {
        return [decoratorMethod.toUpperCase()];
    }
    const explicit = Array.from(methodsText.matchAll(/["'](GET|POST|PUT|DELETE|PATCH|OPTIONS|HEAD)["']/gi)).map((match) => match[1].toUpperCase());
    return explicit.length > 0 ? explicit : ['GET'];
}
function inferPythonFramework(content) {
...
function readPythonFunctionBody(content, startIndex) {
...
function readJsFunctionBody(content, startIndex) {
...
function findServiceCalls(body) {
    const calls = [];
    const callRegex = /\b(?:this\.)?([A-Za-z_][\w]*(?:Service|service|_service)|service|workflow_service|task_service|worker_service)\.([A-Za-z_][\w]*)\s*\(/g;
...
//# sourceMappingURL=httpRouteExtractor.js.map