...
exports.resourceAccessExtractor = void 0;
...
exports.resourceAccessExtractor = {
    name: 'ResourceAccessExtractor',
    detect(context) {
        return context.files.some((file) => RESOURCE_PATTERNS.some((pattern) => pattern.patterns.some((expression) => new RegExp(expression.source, expression.flags).test(file.content))));
    },
    extract(context) {
        const nodes = [];
        const edges = [];
            ...
            for (const resource of RESOURCE_PATTERNS) {
                for (const expression of resource.patterns) {
                    ...
                    nodes.push({
                        ...
                        evidence: matches.map((match) => (0, runtimeUtils_1.lineEvidence)(file, 'import', match[0], match.index, 0.65))
                    });
                    ...
                    nodes.push({
                        ...
                        sourcePath: file.relativePath,
                        category: 'backend',
                        evidence: [(0, runtimeUtils_1.lineEvidence)(file, 'inferred', ownerId.anchor, ownerId.index, 0.5)]
                    });
...
function inferOwnerNode(relativePath, content, accessIndex) {
    ...
    if (latestFunction) {
        ...
        const ownerType = /node|step|stage/i.test(functionName) ? 'WorkflowNode' : /task|worker/i.test(functionName) ? 'Task' : 'Service';
        return {
            id: (0, runtimeUtils_1.normalizeRuntimeId)(`${ownerType}.${functionName}`),
            type: ownerType,
    ...
    if (latestClass) {
        ...
        const ownerType = /node|step|stage/i.test(className) ? 'WorkflowNode' : /task|worker/i.test(className) ? 'Task' : 'Service';
        return {
            id: (0, runtimeUtils_1.normalizeRuntimeId)(`${ownerType}.${className}`),
            type: ownerType,
    ...
    return {
        id: (0, runtimeUtils_1.inferServiceId)(relativePath.replace(/\.[^.]+$/, '')),
        type: 'Service',
        index: 0,
        anchor: relativePath
    };
...
//# sourceMappingURL=resourceAccessExtractor.js.map