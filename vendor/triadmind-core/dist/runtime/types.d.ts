...
export type RuntimeNodeType = 'FrontendEntry' | 'FrontendComponent' | 'ApiRoute' | 'CliCommand' | 'RpcEndpoint' | 'EventConsumer' | 'MessageProducer' | 'Workflow' | 'WorkflowNode' | 'WorkflowEdge' | 'Service' | 'Worker' | 'Task' | 'Queue' | 'Scheduler' | 'DataStore' | 'ObjectStore' | 'Cache' | 'FileSystem' | 'ExternalApi' | 'ExternalTool' | 'ModelProvider' | 'Kernel' | 'Plugin' | 'Config' | 'Secret' | 'UnknownRuntime';
export type RuntimeEdgeType = 'calls' | 'invokes' | 'dispatches' | 'publishes' | 'subscribes' | 'enqueues' | 'consumes' | 'schedules' | 'contains' | 'connects' | 'reads' | 'writes' | 'caches' | 'loads_config' | 'uses_secret' | 'uses_tool' | 'uses_model' | 'executes' | 'returns_to' | 'depends_on';
...
export type RuntimeSourceLanguage = 'python' | 'typescript' | 'javascript' | 'json' | 'yaml' | 'toml' | 'dockerfile' | 'unknown';
