...
exports.GOVERN_EXIT_CODES = void 0;
exports.runGovern = runGovern;
exports.formatGovernReport = formatGovernReport;
...
const REQUIRED_GOVERN_METRICS = [
    ...
    'protocol_focus_alignment_violations',
    'focus_closure_violations',
    'c2c_coupling_count'
];
...
function runGovern(paths, options) {
    const startedAt = Date.now();
    const auditLines = [];
    const report = createBaseReport(paths, options.mode, (0, governPolicy_1.resolveGovernPolicyPath)(paths, options.policyPath));
    const addAudit = (message) => auditLines.push(`[${new Date().toISOString()}] ${message}`);
    ...
    if (!loadedPolicy.policy) {
        ...
        report.exitCode = exports.GOVERN_EXIT_CODES.policy_invalid;
        report.passed = false;
    ...
    if (missingArtifacts.length > 0) {
        ...
        report.exitCode = exports.GOVERN_EXIT_CODES.artifact_missing;
        report.passed = false;
    ...
    const governScope = options.scope ?? config.governance.scope ?? 'full';
    report.scope = governScope;
    const verifyReport = (0, verify_1.runTopologyVerify)(paths, {
        strict: true,
        baselinePath,
        focus: governScope
    });
    const coverageReport = (0, coverage_1.runCoverage)(paths);
    const metrics = { ...verifyReport.metrics };
    ...
    const languagePolicy = {
        violations: (0, topologyQuality_1.evaluateGhostPolicyViolations)(triadNodes, loadedPolicy.policy.languageGhostPolicy, {
            violationLimit: 100
        })
    };
    ...
    report.checks.push({
        ...
        detail: languagePolicy.violations.length > 0
    ...
    if (forbiddenChanges.length > 0) {
        ...
        report.exitCode = exports.GOVERN_EXIT_CODES.gate_fail;
        report.passed = false;
        addAudit('gate fail');
    }
    else {
...
function formatGovernReport(report) {
    ...
    const lines = [
        ...
        `artifacts: report=${report.artifacts.governReportFile}, audit=${report.artifacts.governAuditFile}${report.artifacts.governFixesFile ? `, fixes=${report.artifacts.governFixesFile}` : ''}`
    ];
...
function finalizeGovernRun(paths, report, auditLines, options, startedAt) {
    ...
    if (options.mode === 'fix' && !fs.existsSync(paths.governFixesFile)) {
        fs.mkdirSync(path.dirname(paths.governFixesFile), { recursive: true });
        fs.writeFileSync(paths.governFixesFile, buildFixPatch(report, options, report.baseline?.path ?? paths.verifyBaselineFile), 'utf-8');
    }
...
function createBaseReport(paths, mode, policyPath) {
    return {
        schemaVersion: '1.0',
        ...
        scope: 'full',
        passed: false,
        exitCode: exports.GOVERN_EXIT_CODES.gate_fail,
        checks: [],
        metrics: {},
        artifacts: {
...
function evaluateMustPassRules(policy, metrics, baselinePath) {
    const checks = [];
    const errors = [];
    let baselineCount;
    ...
    return {
        checks,
        errors,
        baseline: typeof baselineCount === 'number'
            ? {
                path: baselinePath,
...
function evaluateSingleRule(key, actual, rule, baselinePath, cachedBaselineCount) {
    if (rule.op === 'lte_baseline_factor') {
        const actualNumber = toFiniteNumber(actual);
        if (typeof actualNumber !== 'number') {
            return {
                check: metricUnavailableCheck(key, `Metric "${key}" is not available for lte_baseline_factor evaluation`, rule.value),
        ...
        if (typeof baselineCount !== 'number') {
            return {
                check: metricUnavailableCheck(key, `Baseline unavailable at ${baselinePath} for lte_baseline_factor`, rule.value),
...
function metricUnavailableCheck(key, detail, expected) {
    return {
        key,
        status: 'error',
...
function validateRuntimeDiagnosticsShape(runtimeDiagnosticsPath) {
    const diagnostics = (0, artifactReaders_1.readRuntimeDiagnosticsArtifact)(runtimeDiagnosticsPath);
    const errors = [];
...
function writeGovernArtifacts(paths, report, auditLines, mode) {
...
function loadGovernPolicy(policyPath) {
    if (!fs.existsSync(policyPath)) {
        return {
            error: `Policy file not found: ${policyPath}`
        };
    }
    const parsed = (0, artifactReaders_1.readJsonIfExists)(policyPath);
    ...
    const policyMustPass = normalizeMustPass(policy.must_pass);
    const defaultMustPass = (0, governPolicy_1.buildDefaultGovernPolicy)().must_pass;
    const mustPass = policyMustPass
        ? {
            ...defaultMustPass,
            ...policyMustPass
        }
...
function normalizeMustPass(raw) {
...
function normalizeLanguageGhostPolicy(raw) {
    if (!raw || typeof raw !== 'object') {
        return undefined;
    }
    const result = {};
    for (const [languageKey, value] of Object.entries(raw)) {
...
function normalizeCoverageByRoot(raw) {
    if (raw === undefined) {
        return {};
    }
    if (!raw || typeof raw !== 'object') {
...
function normalizeForbiddenMutations(raw) {
    if (raw === undefined) {
        return new Set();
    }
...
function resolveBaselinePath(paths, policy, policyPath) {
...
function readBaselineCount(baselinePath) {
    return (0, artifactReaders_1.readVerifyBaselineArtifact)(baselinePath)?.runtime_unmatched_route_count;
}
function captureForbiddenSnapshot(policyPath, baselinePath) {
    return {
        policy: fingerprintFile(policyPath),
        baseline: fingerprintFile(baselinePath)
    };
}
function detectForbiddenChanges(before, after, forbiddenInRun) {
    const changes = [];
...
function buildFixPatch(report, options, baselinePath) {
...
function buildCoverageMetricSnapshot(report) {
    return {
        by_category: Object.fromEntries(Object.entries(report.byCategory).map(([key, bucket]) => [
            key,
            {
        ...
        by_root: Object.fromEntries(Object.entries(report.byRoot).map(([key, bucket]) => [
            key,
            {
                triad: bucket.triadCoverage,
                runtime: bucket.runtimeCoverage,
...
function buildViewMapMetricSnapshot(viewMapPath) {
    const parsed = (0, artifactReaders_1.readJsonIfExists)(viewMapPath);
    const stats = parsed && typeof parsed === 'object' ? parsed.stats : undefined;
...
function evaluateCoverageRules(coverageRules, coverageReport) {
...
function evaluateConfiguredCoverageGates(coverageGates, coverageReport) {
    return normalizeConfiguredCoverageGates(coverageGates).map((gate) => {
        const bucket = gate.scope === 'category'
...
function normalizeConfiguredCoverageGates(coverageGates) {
    const normalized = [];
...
function selectCoverageMetric(bucket, metric) {
    if (metric === 'triad') {
