...
exports.getDreamRuleFilePath = getDreamRuleFilePath;
exports.loadDreamRuleLedger = loadDreamRuleLedger;
exports.saveDreamRuleLedger = saveDreamRuleLedger;
exports.addDreamRule = addDreamRule;
exports.deactivateDreamRule = deactivateDreamRule;
exports.getActiveRules = getActiveRules;
exports.formatActiveRulesPrompt = formatActiveRulesPrompt;
exports.expireStaleRules = expireStaleRules;
exports.jaccardSimilarity = jaccardSimilarity;
exports.findConflictingRules = findConflictingRules;
exports.deduplicateRules = deduplicateRules;
exports.checkForDuplicateBeforeAdd = checkForDuplicateBeforeAdd;
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const artifactReaders_1 = require("./artifactReaders");
const RULE_LEDGER_FILENAME = 'dream-rules.json';
function getDreamRuleFilePath(paths) {
    return path.join(paths.triadDir, RULE_LEDGER_FILENAME);
}
function loadDreamRuleLedger(paths) {
    const filePath = getDreamRuleFilePath(paths);
    const readResult = (0, artifactReaders_1.readJsonObjectArtifactResult)(filePath);
...
function saveDreamRuleLedger(paths, ledger) {
    const filePath = getDreamRuleFilePath(paths);
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, JSON.stringify(ledger, null, 2), 'utf-8');
}
function addDreamRule(paths, input) {
...
function deactivateDreamRule(paths, ruleId) {
    const loadResult = loadDreamRuleLedger(paths);
    let found = false;
...
function getActiveRules(ledger) {
    const now = Date.now();
...
function formatActiveRulesPrompt(ledger) {
...
function expireStaleRules(paths) {
    const loadResult = loadDreamRuleLedger(paths);
    const now = Date.now();
    let expiredCount = 0;
 ...
 */
function findConflictingRules(ledger, newRule) {
    ...
    const processPair = (a, b) => {
        ...
        if (sameScope && first.constraintType !== second.constraintType) {
            const isOpposite = (first.constraintType === 'forbid' && second.constraintType === 'require') ||
    ...
    if (newRule) {
        for (const existing of candidates) {
            if (existing.ruleId === newRule.ruleId)
                continue;
 ...
 */
function deduplicateRules(paths) {
    const loadResult = loadDreamRuleLedger(paths);
    ...
    for (const [, group] of groups) {
        ...
        for (let i = 0; i < group.length; i++) {
            if (toDeactivate.has(group[i].ruleId))
                continue;
            for (let j = i + 1; j < group.length; j++) {
                if (toDeactivate.has(group[j].ruleId))
                    continue;
 ...
 */
function checkForDuplicateBeforeAdd(paths, input) {
    const loadResult = loadDreamRuleLedger(paths);
    ...
    const isExactDuplicate = existingActive.some((r) => r.targetPattern === candidate.targetPattern &&
...
// ─── Internal helpers ───────────────────────────────────────────
function createEmptyDreamRuleLedger(paths) {
    return {
        schemaVersion: '1.0',
...
//# sourceMappingURL=dreamRuleLedger.js.map