...
exports.createJavaScriptAdapter = createJavaScriptAdapter;
exports.createPythonAdapter = createPythonAdapter;
exports.createGoAdapter = createGoAdapter;
exports.createRustAdapter = createRustAdapter;
exports.createCppAdapter = createCppAdapter;
exports.createJavaAdapter = createJavaAdapter;
...
function createJavaScriptAdapter() {
...
function createPolyglotAdapter(descriptor) {
    return {
        ...
        parseTopology: (projectRoot, outputPath, configOverride) => runPolyglotParser(descriptor.language, projectRoot, outputPath, configOverride),
        applyUpgradeProtocol: (projectRoot, protocolPath) => applyPolyglotProtocol(descriptor.language, projectRoot, protocolPath),
        supportsRuntimeHealing: true
    };
}
function readPolyglotTopologyIR(language, projectRoot) {
    const paths = (0, workspace_1.getWorkspacePaths)(projectRoot);
    return (0, ir_1.buildTopologyIR)((0, protocol_1.readTriadMap)(paths.mapFile), language);
}
function runPolyglotParser(language, projectRoot, outputPath, configOverride) {
    const paths = (0, workspace_1.getWorkspacePaths)(projectRoot);
    const config = configOverride ?? (0, config_1.loadTriadConfig)(paths);
    if (config.architecture.parserEngine !== 'native') {
        (0, treeSitterParser_1.runTreeSitterParser)(language, projectRoot, outputPath ?? paths.mapFile, config);
        return;
    }
    const descriptor = DESCRIPTORS[language];
    const nodes = [];
    const targetOutput = outputPath ?? paths.mapFile;
    const includeSourcePath = (0, config_1.createSourcePathFilter)(projectRoot, config);
    const scanScope = (0, config_1.describeSourceScanScope)(projectRoot, config);
    ...
    walkProject(projectRoot, (absolutePath) => {
        const sourcePath = (0, workspace_1.normalizePath)(path.relative(projectRoot, absolutePath));
        ...
        let content;
        try {
            content = (0, artifactReaders_1.readSourceFileText)(absolutePath);
        }
        catch (error) {
...
function applyPolyglotProtocol(language, projectRoot, protocolPath) {
    ...
    for (const action of protocol.actions) {
        ...
        const node = action.op === 'create_child'
            ? action.node
            : {
        ...
        changedFiles.add((0, workspace_1.normalizePath)(path.relative(resolvedProjectRoot, filePath)));
    }
    return {
        changedFiles: Array.from(changedFiles).sort()
    };
}
function parsePolyglotFile(language, content, sourcePath, category, config) {
    switch (language) {
        case 'javascript':
...
function createNode(nodeId, category, sourcePath, demand, answer) {
    const ref = (0, protocol_1.parseNodeRef)(nodeId, category);
    return {
        nodeId,
        category,
        sourcePath,
...
function parseJavaScriptFile(content, sourcePath, category, config) {
    const nodes = [];
    ...
    const arrowRegex = /(?:export\s+)?(?:const|let|var)\s+([A-Za-z_]\w*)\s*=\s*(?:async\s*)?\(([^)]*)\)\s*=>/gm;
    for (const classMatch of content.matchAll(classRegex)) {
        ...
        for (const methodMatch of body.matchAll(methodRegex)) {
            ...
            nodes.push(createNode(`${className}.${methodName}`, category, sourcePath, parseJavaScriptParameters(methodMatch[2]), ['unknown']));
        }
    }
    for (const match of content.matchAll(functionRegex)) {
        ...
        nodes.push(createNode(`${moduleName}.${functionName}`, category, sourcePath, parseJavaScriptParameters(match[2]), ['unknown']));
    }
...
function parseJavaFile(content, sourcePath, category) {
    const nodes = [];
    ...
    for (const classMatch of content.matchAll(classRegex)) {
        ...
        const methodRegex = /(?:(?:public|protected|private)\s+)?(?:(?:static|final|abstract|synchronized)\s+)*([A-Za-z_<>\[\], ?]+?)\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*\{/gm;
        for (const methodMatch of body.matchAll(methodRegex)) {
            const returnType = sanitizeType(methodMatch[1]
            ...
            nodes.push(createNode(`${className}.${methodName}`, category, sourcePath, parseJavaParameters(methodMatch[3]), [returnType]));
        }
    }
    return dedupeNodes(nodes);
}
function parsePythonFile(content, sourcePath, category, config) {
    const nodes = [];
    const moduleName = toPascalCase(path.basename(sourcePath, path.extname(sourcePath)));
...
function parseGoFile(content, sourcePath, category) {
    const nodes = [];
    const moduleName = toPascalCase(path.basename(sourcePath, '.go'));
    const methodRegex = /^func\s*\(\s*[_A-Za-z]\w*\s+\*?([A-Za-z_]\w*)\s*\)\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*(?:\(([^)]*)\)|([^{\s]+))?/gm;
    const functionRegex = /^func\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*(?:\(([^)]*)\)|([^{\s]+))?/gm;
    for (const match of content.matchAll(methodRegex)) {
        nodes.push(createNode(`${match[1]}.${match[2]}`, category, sourcePath, parseGoParameters(match[3]), [sanitizeType(match[4] || match[5] || 'void')]));
    }
    for (const match of content.matchAll(functionRegex)) {
        const functionName = match[1];
        ...
        nodes.push(createNode(`${moduleName}.${functionName}`, category, sourcePath, parseGoParameters(match[2]), [sanitizeType(match[3] || match[4] || 'void')]));
    }
    return dedupeNodes(nodes);
}
function parseRustFile(content, sourcePath, category) {
    const nodes = [];
...
function parseCppFile(content, sourcePath, category) {
    const nodes = [];
    const moduleName = toPascalCase(path.basename(sourcePath, path.extname(sourcePath)));
    const lines = content.split(/\r?\n/);
    let currentClass = null;
    ...
    for (const line of lines) {
        ...
        if (scopedMatch) {
            nodes.push(createNode(`${scopedMatch[2]}.${scopedMatch[3]}`, category, sourcePath, parseCppParameters(scopedMatch[4]), [sanitizeType(scopedMatch[5] || scopedMatch[1])]));
        }
        else {
...
function parsePythonParameters(paramsText) {
...
function parseJavaScriptParameters(paramsText) {
    return splitArguments(paramsText)
        .map((segment) => segment.replace(/=.*/, '').trim())
        .filter(Boolean)
        .map((segment, index) => {
...
function parseGoParameters(paramsText) {
    return splitArguments(paramsText)
...
function parseJavaParameters(paramsText) {
    return splitArguments(paramsText)
...
function parseRustParameters(paramsText) {
    return splitArguments(paramsText)
        .map((segment) => segment.trim())
...
function parseCppParameters(paramsText) {
    return splitArguments(paramsText)
        .map((segment) => segment.replace(/=.*/, '').trim())
        .filter(Boolean)
        .map((segment, index) => {
...
function sliceBraceBody(content, startIndex) {
...
function upsertPolyglotNode(language, projectRoot, node, nodeLocations, action) {
    ...
    const existingContent = (0, artifactReaders_1.readTextIfExists)(filePath);
...
function resolvePolyglotSourcePath(language, projectRoot, ref, node, nodeLocations, action) {
    const relativeSource = node.sourcePath || nodeLocations[node.nodeId] || ('parentNodeId' in action ? nodeLocations[action.parentNodeId] : undefined);
    if (relativeSource) {
...
function buildUpdatedSource(language, filePath, existingContent, node, action) {
    switch (language) {
        case 'javascript':
...
function upsertPythonSource(filePath, existingContent, node, action) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    ...
    if (topLevel) {
        return upsertTopLevelFunction(existingContent, new RegExp(`(^|\\n)def\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildPythonFunctionBlock(functionSignature, node));
    }
    return upsertPythonClassMethod(existingContent, ref.className, ref.methodName, buildPythonMethodBlock(classSignature, node));
}
function upsertJavaScriptSource(filePath, existingContent, node, action) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    ...
    if (topLevel) {
        return upsertTopLevelFunction(existingContent, new RegExp(`(^|\\n)(?:export\\s+)?function\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildJavaScriptFunctionBlock(`${exportPrefix}function ${ref.methodName}(${params}) {`, node));
    }
    return upsertJavaScriptClassMethod(existingContent, ref.className, ref.methodName, buildJavaScriptMethodBlock(`${ref.methodName}(${params}) {`, node));
}
function upsertGoSource(filePath, existingContent, node) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    const params = buildGoParameters(node.fission.demand);
    const returnType = buildGoReturn((0, protocol_1.parseReturnType)(node.fission.answer[0] ?? 'void'));
    ...
    if (shouldUseTopLevel(filePath, existingContent, ref, 'go')) {
        return upsertTopLevelFunction(content, new RegExp(`(^|\\n)func\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildGoFunctionBlock(`func ${ref.methodName}(${params})${returnType} {`, node));
    }
    ...
    return upsertTopLevelFunction(content, new RegExp(`(^|\\n)func\\s*\\(\\s*[A-Za-z_]\\w*\\s+\\*?${escapeRegExp(ref.className)}\\s*\\)\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildGoFunctionBlock(`func (receiver *${ref.className}) ${ref.methodName}(${params})${returnType} {`, node));
}
function upsertRustSource(filePath, existingContent, node) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    ...
    const returnType = buildRustReturn((0, protocol_1.parseReturnType)(node.fission.answer[0] ?? 'void'));
    let content = existingContent;
    if (shouldUseTopLevel(filePath, existingContent, ref, 'rust')) {
        return upsertTopLevelFunction(content, new RegExp(`(^|\\n)(?:pub\\s+)?fn\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildRustFunctionBlock(`pub fn ${ref.methodName}(${params})${returnType} {`, node));
    }
    ...
    const implBlock = `impl ${ref.className} {\n${indentBlock(buildRustFunctionBlock(`pub fn ${ref.methodName}(${methodParams})${returnType} {`, node), 4)}\n}\n`;
    if (!new RegExp(`\\bimpl\\s+${escapeRegExp(ref.className)}\\b`).test(content)) {
        return ensureSeparated(content, implBlock);
    }
    return insertIntoRustImpl(content, ref.className, ref.methodName, buildRustFunctionBlock(`pub fn ${ref.methodName}(${methodParams})${returnType} {`, node));
}
function upsertCppSource(filePath, existingContent, node) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    const params = buildCppParameters(node.fission.demand);
    const returnType = buildCppReturn((0, protocol_1.parseReturnType)(node.fission.answer[0] ?? 'void'));
    let content = ensureCppIncludes(existingContent);
    if (shouldUseTopLevel(filePath, existingContent, ref, 'cpp')) {
        return upsertTopLevelFunction(content, new RegExp(`(^|\\n)(?:auto|[A-Za-z_:][^\\n{;]*)\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildCppFunctionBlock(`auto ${ref.methodName}(${params}) -> ${returnType} {`, node));
    }
    ...
    return upsertTopLevelFunction(content, new RegExp(`(^|\\n)(?:auto|[A-Za-z_:][^\\n{;]*)\\s+${escapeRegExp(ref.className)}::${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), buildCppFunctionBlock(`auto ${ref.className}::${ref.methodName}(${params}) -> ${returnType} {`, node));
}
function upsertJavaSource(filePath, existingContent, node) {
    const ref = (0, protocol_1.parseNodeRef)(node.nodeId, node.category);
    const params = buildJavaParameters(node.fission.demand);
    const returnType = buildJavaReturn((0, protocol_1.parseReturnType)(node.fission.answer[0] ?? 'void'));
    ...
    if (methodPattern.test(classBlock)) {
        return replaceSignatureLine(contentWithImports, new RegExp(`(^|\\n)\\s*(?:public|protected|private)?\\s*(?:static\\s+)?[A-Za-z_<>, ?\\[\\]]+\\s+${escapeRegExp(ref.methodName)}\\s*\\([^\\n]*`, 'm'), `    ${firstLine(methodBlock).trimStart()}`);
    }
    return insertIntoBraceBlock(contentWithImports, classPattern, `\n${indentBlock(methodBlock, 4)}\n`);
}
function shouldUseTopLevel(filePath, content, ref, language, action) {
...
function upsertTopLevelFunction(content, signaturePattern, block) {
...
function upsertPythonClassMethod(content, className, methodName, block) {
...
function upsertJavaScriptClassMethod(content, className, methodName, block) {
    ...
    if (new RegExp(`\\b${escapeRegExp(methodName)}\\s*\\(`, 'm').test(classBody)) {
        return replaceSignatureLine(content, new RegExp(`(^|\\n)\\s*(?:async\\s+)?${escapeRegExp(methodName)}\\s*\\([^\\n]*`, 'm'), `    ${firstLine(block).trimStart()}`);
    }
    return insertIntoBraceBlock(content, classPattern, `\n${indentBlock(block, 4)}\n`);
}
function insertIntoRustImpl(content, className, methodName, functionBlock) {
...
function insertIntoCppClass(content, className, declaration) {
    const classPattern = new RegExp(`(?:class|struct)\\s+${escapeRegExp(className)}\\b[^\\n]*\\{`, 'm');
    return insertIntoBraceBlock(content, classPattern, `\n    ${declaration}\n`);
}
function extractCppClassBlock(content, className) {
    const classPattern = new RegExp(`(?:class|struct)\\s+${escapeRegExp(className)}\\b[^\\n]*\\{`, 'm');
...
function buildPythonFunctionBlock(signature, node) {
    return [
        signature,
...
function buildPythonParameters(demand, includeSelf) {
...
function buildJavaScriptParameters(demand) {
    return buildNamedParameters(demand, '', false, true);
}
function buildGoParameters(demand) {
    return buildNamedParameters(demand, ' ', false);
}
function buildRustParameters(demand, includeReceiver) {
    const params = buildNamedParameters(demand, ': ', false);
    if (includeReceiver) {
...
function buildCppParameters(demand) {
    return buildNamedParameters(demand, ' ', true);
}
function buildJavaParameters(demand) {
    return buildNamedParameters(demand, ' ', true);
}
function buildNamedParameters(demand, delimiter, typeFirst, namesOnly = false) {
    return demand
        .map((entry, index) => (0, protocol_1.parseDemandEntry)(entry, index))
        .filter((value) => Boolean(value))
        .map((value, index) => namesOnly
...
function buildPythonReturn(returnType) {
    return returnType && returnType !== 'void' ? ` -> ${normalizeParameterType(returnType)}` : '';
}
function buildGoReturn(returnType) {
    return returnType && returnType !== 'void' ? ` ${normalizeParameterType(returnType)}` : '';
}
function buildRustReturn(returnType) {
    return returnType && returnType !== 'void' ? ` -> ${normalizeParameterType(returnType)}` : '';
}
function buildCppReturn(returnType) {
    return normalizeParameterType(returnType || 'void');
}
function buildJavaReturn(returnType) {
    return normalizeParameterType(returnType || 'void');
}
function normalizeParameterType(typeText) {
    return sanitizeType(typeText || 'unknown');
}
function ensureCppIncludes(content) {
    let next = content;
...
function ensureJavaImports(content, node) {
    ...
    node.fission.demand.forEach((entry, index) => {
        const parsed = (0, protocol_1.parseDemandEntry)(entry, index);
        if (parsed) {
            registerType(parsed.type);
        }
    });
    registerType((0, protocol_1.parseReturnType)(node.fission.answer[0] ?? 'void'));
    let next = content;
...
function replaceSignatureLine(content, pattern, signatureLine) {
...
function insertIntoIndentedBlock(content, startPattern, insertion) {
...
function insertIntoBraceBlock(content, startPattern, insertion) {
    const match = startPattern.exec(content);
    if (!match || match.index < 0) {
        return ensureSeparated(content, insertion.trim());
    }
...
function locateBraceBlock(content, braceIndex) {
...
function ensureSeparated(content, block) {
...
function matchOrDefault(value, fallback) {
    return value && value.trim() ? value.trim() : fallback;
}
//# sourceMappingURL=polyglotAdapter.js.map