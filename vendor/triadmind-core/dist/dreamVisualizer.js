...
exports.generateDreamDashboard = generateDreamDashboard;
exports.generateDreamDashboardFromFile = generateDreamDashboardFromFile;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const artifactReaders_1 = require("./artifactReaders");
function generateDreamDashboard(report, outputFilePath, options = {}) {
    fs.mkdirSync(path.dirname(outputFilePath), { recursive: true });
    const html = renderDreamDashboardHtml(report, options);
    fs.writeFileSync(outputFilePath, html, 'utf-8');
}
function generateDreamDashboardFromFile(reportFilePath, outputFilePath, options = {}) {
    const report = readDreamReport(reportFilePath);
    generateDreamDashboard(report, outputFilePath, options);
}
function readDreamReport(reportFilePath) {
    const result = (0, artifactReaders_1.readJsonObjectArtifactResult)(reportFilePath);
    if (result.status !== 'ok' || !result.value) {
        throw new Error(`Invalid dream report file: ${reportFilePath}`);
    }
    return result.value;
}
function renderDreamDashboardHtml(report, options) {
...
//# sourceMappingURL=dreamVisualizer.js.map