...
exports.BootstrapScaffoldService = void 0;
...
class BootstrapScaffoldService {
    init(paths, options = {}) {
        ...
        results.push(writeTemplateFile(paths.skillsFile, templates.skills, {
            force: Boolean(options.force),
            key: 'skills.md',
            projectRoot: paths.projectRoot
        }));
        results.push(writeTemplateFile(paths.sessionBootstrapShellFile, templates.sessionBootstrapShell, {
            force: Boolean(options.force),
            key: 'session-bootstrap.sh',
            executable: true,
            projectRoot: paths.projectRoot
        }));
        results.push(writeTemplateFile(paths.sessionBootstrapPs1File, templates.sessionBootstrapPs1, {
            force: Boolean(options.force),
            key: 'session-bootstrap.ps1',
            projectRoot: paths.projectRoot
        }));
        results.push(writeTemplateFile(paths.sessionBootstrapCmdFile, templates.sessionBootstrapCmd, {
            force: Boolean(options.force),
    ...
    doctor(paths, options = {}) {
        ...
        statuses.push(inspectTemplateFile(paths.sessionBootstrapShellFile, templates.sessionBootstrapShell, 'session-bootstrap.sh', {
            requireExecutable: true,
            projectRoot: paths.projectRoot
        }));
        statuses.push(inspectTemplateFile(paths.sessionBootstrapPs1File, templates.sessionBootstrapPs1, 'session-bootstrap.ps1', {
            projectRoot: paths.projectRoot
        }));
        statuses.push(inspectTemplateFile(paths.sessionBootstrapCmdFile, templates.sessionBootstrapCmd, 'session-bootstrap.cmd', {
            projectRoot: paths.projectRoot
        }));
    ...
    upsertAgentsFile(paths, managedBlock) {
        const targetPath = paths.agentsFile;
        const existed = fs.existsSync(targetPath);
        const existing = (0, artifactReaders_1.readTextIfExists)(targetPath);
        const next = upsertManagedRulesBlock(existing, managedBlock);
        if (normalizeLineEndings(existing) === normalizeLineEndings(next)) {
            return {
                key: 'AGENTS.md',
                path: (0, workspace_1.normalizePath)(path.relative(paths.projectRoot, targetPath)),
                action: 'skipped',
                existed,
                changed: false,
                executable: false
    ...
    inspectAgents(paths, managedBlock) {
        const targetPath = paths.agentsFile;
        ...
        const content = (0, artifactReaders_1.readRequiredTextFile)(targetPath);
...
exports.BootstrapScaffoldService = BootstrapScaffoldService;
function writeTemplateFile(targetPath, content, options) {
    const existed = fs.existsSync(targetPath);
...
function inspectTemplateFile(targetPath, expectedContent, key, options = {}) {
    const resolvedPath = options.projectRoot
        ? toProjectRelativePath(options.projectRoot, targetPath)
        : (0, workspace_1.normalizePath)(targetPath);
    if (!fs.existsSync(targetPath)) {
        return {
            key,
            path: resolvedPath,
    ...
    const current = (0, artifactReaders_1.readRequiredTextFile)(targetPath);
    ...
    return {
        ...
        recommendedAction: status === 'pass'
            ...
            : options.requireExecutable && !executable && upToDate
                ? `Run \`chmod +x ${(0, workspace_1.normalizePath)(targetPath)}\`.`
...
function toProjectRelativePath(projectRoot, targetPath) {
    return (0, workspace_1.normalizePath)(path.relative(projectRoot, targetPath));
}
function upsertManagedRulesBlock(existing, managedBlock) {
...
function extractManagedRulesBlock(content) {
    const pattern = new RegExp(`${escapeRegExp(START_MARKER)}[\\s\\S]*?${escapeRegExp(END_MARKER)}`, 'm');
...
function hasExecutableBit(targetPath) {
...
function ensureExecutable(targetPath, required) {
...
function loadBootstrapTemplateFile(key) {
    const filenameByKey = {
    ...
    for (const candidate of candidates) {
        if (fs.existsSync(candidate)) {
            return (0, artifactReaders_1.readRequiredTextFile)(candidate);
        }
    }
...
//# sourceMappingURL=bootstrapScaffoldService.js.map