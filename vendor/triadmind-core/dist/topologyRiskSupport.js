...
exports.hasGhostDemand = hasGhostDemand;
exports.isGhostNoiseNode = isGhostNoiseNode;
exports.isSupportLayerAggregateNode = isSupportLayerAggregateNode;
exports.isHelperPrimitiveNode = isHelperPrimitiveNode;
exports.isStructuralRiskCandidate = isStructuralRiskCandidate;
exports.isMatureStableArchitectureNode = isMatureStableArchitectureNode;
exports.getPromotionReasons = getPromotionReasons;
...
function isGhostNoiseNode(node, degree, executeLike = false) {
...
function isSupportLayerAggregateNode(node) {
    const nodeId = String(node?.nodeId ?? '').trim();
    if (!/\.module_pipeline$/i.test(nodeId)) {
        return false;
    }
    return (0, supportAbstraction_1.isSupportAbstractionSourcePath)(String(node?.sourcePath ?? ''));
}
function isHelperPrimitiveNode(node) {
    ...
    const supportSource = (0, supportAbstraction_1.isSupportAbstractionSourcePath)(String(node?.sourcePath ?? ''));
...
function isStructuralRiskCandidate(node) {
    return !isSupportLayerAggregateNode(node) && !isHelperPrimitiveNode(node) && !isAdministrativeAggregateNode(node);
}
function isMatureStableArchitectureNode(node, options) {
    const nodeId = String(node?.nodeId ?? '').trim();
    const sourcePath = normalizeRiskSourcePath(String(node?.sourcePath ?? ''));
    if (!nodeId && !sourcePath) {
        return false;
    }
    const matureStableNodeIds = new Set((options?.matureStableNodeIds ?? []).map((entry) => String(entry ?? '').trim()).filter(Boolean));
    if (nodeId && matureStableNodeIds.has(nodeId)) {
        return true;
    }
    const matureStableSourcePaths = new Set((options?.matureStableSourcePaths ?? [])
...
function isAdministrativeAggregateNode(node) {
    const nodeId = String(node?.nodeId ?? '').trim();
...
function hasCapabilityNamePrefix(name, prefix) {
...
function normalizeRiskSourcePath(value) {
    return String(value ?? '')
        .trim()
        .replace(/\\/g, '/')
        .replace(/^\.?\//, '')
...
function matchesRegexPattern(value, pattern) {
...
function matchesSourcePathPattern(sourcePath, pattern) {
...
//# sourceMappingURL=topologyRiskSupport.js.map