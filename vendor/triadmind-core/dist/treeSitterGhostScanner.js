...
exports.scanTreeSitterGhostReferences = scanTreeSitterGhostReferences;
...
function scanTreeSitterGhostReferences(executableNode, options = {}) {
    const profile = mergeOptions(options);
    const bindingProfile = createBindingProfile(profile);
    const parameters = new Set(extractParameterNames(executableNode, profile, bindingProfile));
    const locals = new Set(parameters);
    const bodyNode = findFunctionBody(executableNode, profile) ?? executableNode;
    const references = new Map();
    collectLocalNames(bodyNode, locals, profile, bindingProfile);
...
function createBindingProfile(profile) {
    return {
        identifierNodes: profile.identifierNodes,
        memberExpressionNodes: profile.memberExpressionNodes,
        skipNodeTypes: ['subscript'],
        dedupe: true,
        mapIdentifierText: normalizeName,
        isMemberPropertyName: (node) => isMemberPropertyName(node, profile)
    };
}
function collectLocalNames(rootNode, locals, profile, bindingProfile) {
    for (const node of walkNodes(rootNode)) {
        ...
        const nameNode = node.childForFieldName('name') ?? node.namedChildren[0] ?? null;
        for (const name of (0, treeSitterBindingSupport_1.extractBindingNames)(nameNode, bindingProfile)) {
            locals.add(name);
        }
    }
}
function extractParameterNames(executableNode, profile, bindingProfile) {
    const parametersNode = executableNode.childForFieldName('parameters') ??
        executableNode.namedChildren.find((node) => profile.parameterNodes.includes(node.type)) ??
        null;
    if (!parametersNode) {
        return [];
    }
    return (0, treeSitterBindingSupport_1.extractParameterBindingNames)(parametersNode, bindingProfile);
}
function findFunctionBody(executableNode, profile) {
...
function extractMemberGhostReference(node, parameters, locals, profile) {
...
function getMemberRoot(node, profile) {
    let current = node;
    while (current && isMemberExpression(current, profile)) {
        const objectNode = current.namedChildren[0] ?? null;
        if (!objectNode) {
            return null;
        }
...
function getGhostAccessMode(node, profile) {
...
function getAccessTarget(node, profile) {
    let current = node;
...
function mergeReference(references, reference) {
...
function mergeAccessMode(left, right) {
    if (left === right) {
        return left;
    }
...
function isOutermostMemberExpression(node, profile) {
    return !(node.parent && isMemberExpression(node.parent, profile) && node.parent.namedChildren[0]?.id === node.id);
}
function isMemberRootIdentifier(node, profile) {
    return !!node.parent && isMemberExpression(node.parent, profile) && node.parent.namedChildren[0]?.id === node.id;
}
function isDeclarationName(node, profile) {
    const parent = node.parent;
    if (!parent || !isIdentifierNode(node, profile)) {
        return false;
    }
    return (((parent.type === 'variable_declarator' || parent.type === 'pair_pattern') && parent.namedChildren[0]?.id === node.id) ||
...
//# sourceMappingURL=treeSitterGhostScanner.js.map