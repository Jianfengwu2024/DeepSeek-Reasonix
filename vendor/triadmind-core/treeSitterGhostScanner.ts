...
function createBindingProfile(profile: Required<TreeSitterGhostScanOptions>): TreeSitterBindingProfile {
    return {
...
function collectLocalNames(
    ...
    bindingProfile: TreeSitterBindingProfile
) {
    for (const node of walkNodes(rootNode)) {
        ...
        const nameNode = node.childForFieldName('name') ?? node.namedChildren[0] ?? null;
        for (const name of extractTreeSitterBindingNames(nameNode, bindingProfile)) {
            locals.add(name);
        }
    }
}
...
function extractParameterNames(
    ...
    return extractParameterBindingNames(parametersNode, bindingProfile);
}
