...
import {
    ...
    loadAbstractionMemory,
    type AbstractionMemoryArtifact,
    type AbstractionMemoryEntry,
    type AbstractionMemoryEntryKind
} from './abstractionMemory';
import { readJsonObjectArtifactResult } from './artifactReaders';
import { loadTriadConfig } from './config';
import type { WorkspacePaths } from './workspace';

export type ProjectAbsToolkitEntryKind =
...
export function syncProjectAbsToolkitFromMemory(paths: WorkspacePaths) {
    const memoryArtifact = loadAbstractionMemory(paths.abstractionMemoryFile);
    if (!memoryArtifact) {
...
export function ensureProjectAbsToolkit(paths: WorkspacePaths, options: { force?: boolean } = {}) {
    const existing = loadProjectAbsToolkit(paths.projectAbsToolkitFile);
...
export function loadProjectAbsToolkit(filePath: string) {
    const result = readJsonObjectArtifactResult<ProjectAbsToolkitArtifact>(filePath);
...
export function searchProjectAbsToolkit(artifact: ProjectAbsToolkitArtifact, query: string, limit = 10) {
    const terms = tokenize(query);
    if (terms.length === 0 || artifact.entries.length === 0) {
        return [] as ProjectAbsToolkitSearchResult[];
    }
    ...
    const scored = artifact.entries
        .map((entry) => {
            ...
            return {
                ...
                score,
                matchedTerms
            } satisfies ProjectAbsToolkitSearchResult;
        })
...
export function buildProjectAbsToolkitPromptContext(paths: WorkspacePaths, demand: string): ProjectAbsToolkitPromptContext {
...
export function formatProjectAbsToolkitPromptJson(entries: ProjectAbsToolkitEntry[]) {
...
export function renderProjectAbsToolkitMarkdown(artifact: ProjectAbsToolkitArtifact) {
    const lines: string[] = [
    ...
    artifact.entries.forEach((entry, index) => {
        lines.push(
            '',
            `### ${index + 1}. ${entry.name} [${entry.kind}]`,
            '',
            `- Status: ${entry.status}`,
            `- Reuse policy: ${entry.reusePolicy}`,
            `- Stability: ${entry.stability}`,
            `- Toolkit path: ${entry.category}/${entry.subcategory}`,
            `- Source: ${entry.primarySourcePath}`,
            `- Score: reuse=${entry.reusabilityScore.toFixed(2)}, abstraction=${entry.abstractionRatio.toFixed(2)}`,
            `- Intent: ${entry.intent}`,
            `- Why reusable: ${entry.whyReusable}`
        );
...
export function exportProjectAbsToolkitMarkdown(artifact: ProjectAbsToolkitArtifact, markdownFile: string) {
...
export function exportProjectAbsToolkitDirectory(artifact: ProjectAbsToolkitArtifact, toolkitDir: string) {
    ...
    for (const [category, subgroups] of grouped) {
        ...
        for (const [subcategory, entries] of subgroups) {
            ...
            fs.writeFileSync(
                path.join(subcategoryDir, 'README.md'),
                renderSubcategoryMarkdown(artifact, category, subcategory, entries),
                'utf-8'
            );
...
export function persistProjectAbsToolkit(paths: WorkspacePaths, artifact: ProjectAbsToolkitArtifact) {
...
export function recalculateProjectAbsToolkitSummary(
...
function ensureProjectAbsToolkitForPrompt(paths: WorkspacePaths) {
    ...
    ensureAbstractionMemory(paths, { autoSync: true });
    return ensureProjectAbsToolkit(paths, { force: true });
}

function buildProjectAbsToolkitArtifact(
    paths: WorkspacePaths,
    memoryArtifact: AbstractionMemoryArtifact,
    existingArtifact: ProjectAbsToolkitArtifact | undefined,
    projectName: string
    ...
    const artifact: ProjectAbsToolkitArtifact = {
        schemaVersion: '1.0',
...
function buildToolkitEntryFromMemory(entry: AbstractionMemoryEntry, existing?: ProjectAbsToolkitEntry): ProjectAbsToolkitEntry {
...
function shouldPromoteMemoryEntry(entry: AbstractionMemoryEntry) {
...
function buildDefaultIntent(entry: AbstractionMemoryEntry) {
    return `Reuse ${entry.name} before creating new ${entry.kind === 'abstract_function' ? 'function contracts' : 'abstraction branches'}.`;
}

function buildDefaultApplicability(entry: AbstractionMemoryEntry) {
...
function buildDefaultExamples(entry: AbstractionMemoryEntry) {
    if (entry.signatures.length > 0) {
...
function deriveReusePolicy(kind: ProjectAbsToolkitEntryKind): ProjectAbsToolkitReusePolicy {
...
function deriveStability(entry: AbstractionMemoryEntry): ProjectAbsToolkitStability {
...
function deriveDefaultTaxonomy(kind: ProjectAbsToolkitEntryKind) {
    switch (kind) {
...
function toToolkitEntryKind(kind: AbstractionMemoryEntryKind): ProjectAbsToolkitEntryKind {
    return kind;
}

function scoreToolkitEntry(entry: ProjectAbsToolkitEntry, terms: string[]) {
...
function buildToolkitSearchHaystack(entry: ProjectAbsToolkitEntry) {
    return [
        entry.name,
        entry.kind,
        entry.category,
...
function fuzzyContains(haystack: string, term: string) {
    return (
        haystack.includes(term.replace(/_/g, '')) ||
        haystack.includes(term.replace(/-/g, '')) ||
        haystack.includes(term.replace(/\s+/g, ''))
    );
}

function groupEntriesByCategory(entries: ProjectAbsToolkitEntry[]) {
    const grouped = new Map<string, Map<string, ProjectAbsToolkitEntry[]>>();
    for (const entry of entries) {
        const category = sanitizeTaxonomyComponent(entry.category);
        const subcategory = sanitizeTaxonomyComponent(entry.subcategory);
        const categoryMap = grouped.get(category) ?? new Map<string, ProjectAbsToolkitEntry[]>();
...
function renderCategoryMarkdown(
    artifact: ProjectAbsToolkitArtifact,
    category: string,
    subgroups: Map<string, ProjectAbsToolkitEntry[]>
) {
    ...
    for (const [subcategory, entries] of subgroups) {
        ...
        for (const entry of entries) {
            lines.push(
                `- [${entry.name}](${toPosixPath(path.join(subcategory, `${slugify(entry.id)}.md`))}) [${entry.kind}] policy=${entry.reusePolicy}`
            );
        }
    }
    return lines.join('\n');
...
function renderSubcategoryMarkdown(
    artifact: ProjectAbsToolkitArtifact,
    category: string,
    subcategory: string,
    entries: ProjectAbsToolkitEntry[]
...
function renderEntryMarkdown(entry: ProjectAbsToolkitEntry) {
    const lines = [
        `# ${entry.name}`,
        '',
        `- id: ${entry.id}`,
...
function sanitizeTaxonomyComponent(value: string) {
