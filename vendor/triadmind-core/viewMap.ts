...
import { readJsonArrayArtifactResult, readRuntimeMapArtifactResult } from './artifactReaders';
import { loadTriadConfig, resolveCategoryFromConfig, TriadConfig } from './config';
...
export function generateViewMap(paths: ViewMapPaths, options: ViewMapOptions = {}): ViewMap {
...
export function writeViewMapArtifacts(paths: ViewMapPaths, options: ViewMapOptions = {}) {
...
function readTriadNodes(filePath: string, diagnostics: ViewMapDiagnostic[], missingCode: string) {
    const result = readJsonArrayArtifactResult<TriadNode>(filePath);
    ...
    return result.value ?? ([] as TriadNode[]);
}
...
function resolveFirstMappedCategory(sourcePaths: string[], config: TriadConfig) {
    for (const sourcePath of sourcePaths) {
        const resolved = resolveCategoryFromConfig(sourcePath, config);
        if (resolved !== 'unknown') {
            return resolved;
        }
    }
