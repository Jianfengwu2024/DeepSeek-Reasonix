...
import { evaluateGhostPolicyViolations } from './topologyQuality';
...
export function runGovern(paths: WorkspacePaths, options: GovernRunOptions): GovernExecutionResult {
    ...
    const languagePolicy = {
        violations: evaluateGhostPolicyViolations(triadNodes, loadedPolicy.policy.languageGhostPolicy, {
            violationLimit: 100
        })
    };
...
function validateRuntimeDiagnosticsShape(runtimeDiagnosticsPath: string) {
    const diagnostics = readRuntimeDiagnosticsArtifact(runtimeDiagnosticsPath);
    const errors: string[] = [];
...
function loadGovernPolicy(policyPath: string): { policy?: GovernPolicyNormalized; error?: string } {
    ...
    const defaultMustPass = buildDefaultGovernPolicy().must_pass;
...
function readBaselineCount(baselinePath: string) {
    return readVerifyBaselineArtifact(baselinePath)?.runtime_unmatched_route_count;
}
