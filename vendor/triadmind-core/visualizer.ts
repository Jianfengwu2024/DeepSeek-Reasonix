...
export interface VisualizerGraph {
    nodes: Array<KnowledgeNode & { degree: number; hidden: boolean }>;
    edges: Array<KnowledgeEdge>;
    legend: Array<{ cid: string; label: string; color: string; count: number }>;
    stats: {
        nodes: number;
        edges: number;
        vertices: number;
...
export function generateImpactDashboard(
    mapPath: string,
    protocol: UpgradeProtocol,
    outputPath: string,
    dashboardOptions: DashboardOptions = {}
): ImpactDashboardRenderResult {
...
function generateVisualizerDashboard(
    mapPath: string,
    protocolInput: string | UpgradeProtocol,
    outputPath: string,
    dashboardOptions: DashboardOptions,
    mode: VisualizerMode
) {
    ...
    const protocol = typeof protocolInput === 'string' ? readJsonFile<UpgradeProtocol>(protocolPath) : protocolInput;
    ...
    console.log(
        `[TriadMind] Visualizer mode: view=${options.defaultView} renderMode=${mode} strictFingerprint=${options.strictFingerprint} fastFallback=${mayaData.strictFingerprintSkipped} nodes=${graph.stats.nodes} edges=${graph.stats.edges}`
    );
...
function buildKnowledgeGraph(
    ...
    mode: VisualizerMode = 'review'
) {
    const nodeMap = new Map<string, KnowledgeNode>();
    const edges: KnowledgeEdge[] = [];
    const previewMap = buildPreviewTopology(originalMap, protocol, mode);
    const highlightedOwnerIds = collectHighlightedOwnerIds(protocol, renormalizeProtocol);
    const baseContractEdgeKeys = new Set(
        calculateProducerConsumerEdges(originalMap, options.analyzer).map((edge) => `${edge.from}::${edge.to}::${edge.contract}`)
    );
    ...
    const previewContractEdges = calculateProducerConsumerEdges(previewMap, options.analyzer)
        ...
        .sort(
            (left, right) =>
                Number(right.lifecycle === 'proposed') - Number(left.lifecycle === 'proposed') ||
...
function toKnowledgeNode(node: TriadMapNode, status: NodeStatus, lifecycle: TriadLifecycle = node.lifecycle ?? 'existing'): KnowledgeNode {
...
function upsertTriadVertex(
    ...
    status: NodeStatus,
    includeBranches = true,
    lifecycle: TriadLifecycle = node.lifecycle ?? 'existing'
) {
...
function edgeStyle(edge: KnowledgeEdge) {
    ...
    if (edge.type === 'triad_left') return { color: '#86efac', highlight: '#ffffff', width: edge.highlighted ? 3.8 : 2.8, opacity: 0.96, dashes: false };
    if (edge.type === 'triad_right') return { color: '#e9d5ff', highlight: '#ffffff', width: edge.highlighted ? 3.8 : 2.8, opacity: 0.96, dashes: [4, 3] };
    if (edge.type === 'producer_consumer') return { color: '#cbd5e1', highlight: '#ffffff', width: edge.highlighted ? 3.6 : 2.8, opacity: edge.highlighted ? 0.98 : 0.94, dashes: [8, 4] };
    if (edge.type === 'create_child') return { color: '#67e8f9', highlight: '#ffffff', width: 5.8, opacity: 1, dashes: false };
    if (edge.type === 'protocol_target') return { color: '#e9d5ff', highlight: '#ffffff', width: 3.8, opacity: 0.96, dashes: [8, 5] };
    if (edge.type === 'modify') return { color: '#fdba74', highlight: '#ffffff', width: 3.8, opacity: 0.96, dashes: false };
    if (edge.type === 'renormalize_absorb') return { color: '#fda4af', highlight: '#ffffff', width: 4.8, opacity: 0.98, dashes: [10, 4] };
    if (edge.type === 'renormalize_contract') return { color: '#fecdd3', highlight: '#ffffff', width: 3.2, opacity: 0.94, dashes: [3, 4] };
...
function buildPreviewTopology(originalMap: TriadMapNode[], protocol: UpgradeProtocol, mode: VisualizerMode = 'review') {
...
function buildVisualizerOptions(
    ...
    return {
        analyzer: resolveAnalyzerOptionsFromConfig(config),
