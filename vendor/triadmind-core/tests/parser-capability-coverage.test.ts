...
test('typescript parser attaches abstraction evidence for interfaces, type contracts, abstract bases, and variants', () => {
    const { triadMap, leafMap } = parseFixture('typescript', {
        'backend/payments/strategies.ts': `
interface PaymentStrategy {
    submit(input: PayInput): PayResult;
}
...
type PayInput = { amount: number };
type PayResult = { ok: boolean };

abstract class BasePaymentProvider {
    abstract submit(input: PayInput): PayResult;
}

class StripePay extends BasePaymentProvider implements PaymentStrategy {
    submit(input: PayInput): PayResult {
        return { ok: input.amount > 0 };
    }
}

class PaypalPay extends BasePaymentProvider implements PaymentStrategy {
    submit(input: PayInput): PayResult {
        return { ok: input.amount > 0 };
    }
}

export function runPayment(strategy: PaymentStrategy, input: PayInput): PayResult {
    return strategy.execute(input);
}
`
    });

    const capabilityNodes = triadMap.filter((node) => node.sourcePath === 'backend/payments/strategies.ts');
    const leafNodes = leafMap.filter((node) => node.sourcePath === 'backend/payments/strategies.ts');
    const stripe = leafNodes.find((node) => node.nodeId === 'StripePay.submit');
    const paypal = leafNodes.find((node) => node.nodeId === 'PaypalPay.submit');
    const runner = leafNodes.find((node) => node.nodeId === 'Strategies.runPayment');

    assert.ok(stripe, `expected StripePay.submit leaf node, got ${JSON.stringify(leafNodes, null, 2)}`);
    assert.ok(paypal, `expected PaypalPay.submit leaf node, got ${JSON.stringify(leafNodes, null, 2)}`);
    assert.ok(runner, `expected Strategies.runPayment leaf node, got ${JSON.stringify(leafNodes, null, 2)}`);
    assert.equal(capabilityNodes.some((node) => node.nodeId === 'Strategies.module_pipeline'), true);

    assert.equal(stripe.fission?.evidence?.abstraction?.role, 'concrete');
    assert.deepEqual(stripe.fission?.evidence?.abstraction?.implements, ['PaymentStrategy']);
    assert.deepEqual(stripe.fission?.evidence?.abstraction?.extendsAbstract, ['BasePaymentProvider']);
    assert.equal(stripe.fission?.evidence?.abstraction?.variantCluster, 'pay');
    assert.equal(stripe.fission?.evidence?.abstraction?.abstractionSignalCount, 3);
    assert.equal(stripe.fission?.evidence?.abstraction?.interfaceCount, 1);
    assert.equal(stripe.fission?.evidence?.abstraction?.typeAliasCount, 1);
    assert.equal(stripe.fission?.evidence?.abstraction?.abstractClassCount, 1);
    assert.equal(stripe.fission?.evidence?.abstraction?.signals?.includes('implements_contract'), true);
    assert.equal(stripe.fission?.evidence?.abstraction?.signals?.includes('variant_member'), true);

    assert.equal(paypal.fission?.evidence?.abstraction?.variantCluster, 'pay');
    assert.equal(paypal.fission?.evidence?.abstraction?.signals?.includes('extends_abstract'), true);

    assert.equal(runner.fission?.evidence?.abstraction?.role, 'mixed');
    assert.deepEqual(runner.fission?.evidence?.abstraction?.dependsOnAbstractions, ['PaymentStrategy']);
    assert.equal(runner.fission?.evidence?.abstraction?.signals?.includes('depends_on_contract'), true);
});
