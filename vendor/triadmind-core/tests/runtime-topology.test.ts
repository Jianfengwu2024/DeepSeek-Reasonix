...
test('external frontend API calls are ignored by internal route diagnostics', async () => {
    const root = writeFixture({
        'frontend/src/pages/external.tsx': `
fetch("https://api.github.com/repos/deepseek-ai/DeepSeek-TUI");
fetch("https://raw.githubusercontent.com/deepseek-ai/DeepSeek-TUI/main/README.md");
...
`
    ...
    assert.equal(runtimeMap.edges.some((edge) => edge.type === 'calls' && edge.to.startsWith('ApiRoute.')), false);
});

test('external frontend API calls do not suppress unmatched internal route diagnostics', async () => {
    const root = writeFixture({
        'frontend/src/pages/mixed.tsx': `
fetch("https://api.github.com/repos/deepseek-ai/DeepSeek-TUI");
...
`
    ...
    const runtimeMap = await extractRuntimeTopology(root, { includeFrontend: true });
    const unmatchedDiagnostics = (runtimeMap.diagnostics ?? []).filter(
        (diagnostic) => diagnostic.code === 'RUNTIME_FRONTEND_API_ROUTE_UNMATCHED'
    );

    assert.equal(unmatchedDiagnostics.length, 1);
    assert.match(unmatchedDiagnostics[0]?.message ?? '', /raw=\/api\/unknown\/path/);
    assert.ok(runtimeMap.edges.some((edge) => edge.type === 'calls' && edge.to === 'ApiRoute.UNKNOWN./api/unknown/path'));
});
