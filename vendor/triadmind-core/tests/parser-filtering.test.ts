...
test('scan plan keeps leaf projection while using capability architecture sampling', () => {
    const parserConfig = {
        ...createTestConfig().parser,
        scanMode: 'leaf'
    } as TriadConfig['parser'];
    const plan = createTreeSitterScanPlan(parserConfig);

    assert.equal(plan.projectedScanMode, 'leaf');
    assert.equal(plan.architectureScanMode, 'capability');
    assert.equal(plan.leafParserConfig.scanMode, 'leaf');
    assert.equal(plan.architectureParserConfig.scanMode, 'capability');
    assert.equal(plan.useLeafProjection, true);
    assert.equal(plan.scanUnit, 'leaf nodes');
});

test('scan plan preserves aggregate scan modes for module and domain views', () => {
    const baseParserConfig = createTestConfig().parser;
    const modulePlan = createTreeSitterScanPlan({
        ...baseParserConfig,
        scanMode: 'module'
    });
    const domainPlan = createTreeSitterScanPlan({
        ...baseParserConfig,
        scanMode: 'domain'
    });

    assert.equal(modulePlan.architectureScanMode, 'module');
    assert.equal(modulePlan.architectureParserConfig.scanMode, 'module');
    assert.equal(modulePlan.useLeafProjection, false);
    assert.equal(modulePlan.scanUnit, 'module capability nodes');

    assert.equal(domainPlan.architectureScanMode, 'domain');
    assert.equal(domainPlan.architectureParserConfig.scanMode, 'domain');
    assert.equal(domainPlan.useLeafProjection, false);
    assert.equal(domainPlan.scanUnit, 'domain capability nodes');
});
...
test('root support helpers stay aggregated instead of becoming top-level capabilities', () => {
    const nodes = parseFixture({
        'graphSupport.py': `
def unwrap_definition(node: SyntaxNode) -> SyntaxNode | None:
    return node

def create_node(node_id: str, payload: NodePayload) -> NodeDefinition:
    return payload
`
    });

    assert.deepEqual(nodes.map((node) => node.nodeId), ['GraphSupport.module_pipeline']);
});
