...
test('runtime visualizer v2 html contains interactive graph bootstrap', () => {
    ...
    const frontendNode = payload.nodes.find((node: any) => node.id === 'FrontendEntry.frontend/src/pages/items.tsx');
    const callsEdge = payload.edges.find((edge: any) => edge._type === 'calls');
    ...
    assert.match(html, /#34506f/);
    assert.equal(frontendNode?.color?.background, '#2563eb');
    assert.equal(frontendNode?.color?.border, '#93c5fd');
    assert.equal(callsEdge?.color?.color, '#7dd3fc');
    assert.equal(callsEdge?.color?.opacity, 0.94);
});
