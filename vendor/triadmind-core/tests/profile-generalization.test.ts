...
test('scoped parser and coverage keep root support abstractions inside support-layer aggregate fallback', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'triadmind-profile-support-'));
    const triadDir = path.join(root, '.triadmind');
...
export function resolveAnalyzerOptionsFromConfig(config: AnalyzerConfigLike) {
    return {
...
`
    ...
    const paths = getWorkspacePaths(root);
    const config = loadTriadConfig(paths);
    runTreeSitterParser('typescript', root, paths.mapFile, config);
    const triadMap = JSON.parse(fs.readFileSync(paths.mapFile, 'utf-8')) as Array<{
        nodeId: string;
        category: string;
        sourcePath: string;
    ...
    const supportNodes = triadMap.filter((node) => node.sourcePath === 'analyzerOptionsSupport.ts');
    assert.deepEqual(
        supportNodes.map((node) => `${node.category}:${node.nodeId}`),
        ['core:AnalyzerOptionsSupport.module_pipeline']
    );

    const coverageReport = runCoverage(paths);
    assert.equal(coverageReport.summary.combinedCoverage, 1);
    assert.equal(coverageReport.byCategory.core.combinedCoverage, 1);
    assert.deepEqual(coverageReport.summary.coveredSamples, ['analyzerOptionsSupport.ts']);
});
