...
test('dream run --visualize writes dream-visualizer html', () => {
    ...
    const result = runCli(root, ['dream', 'run', '--visualize', '--theme', 'runtime-dark']);
    ...
    assert.match(html, /--card-bg: #0f1b33;/);
    assert.match(html, /--surface-bg: #12233d;/);
    assert.match(html, /--border: #34506f;/);
});
