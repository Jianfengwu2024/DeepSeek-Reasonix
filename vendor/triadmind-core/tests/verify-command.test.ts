...
function createAbstractionDeficitTriadMap() {
    return [
        {
            nodeId: 'StripePay.submit',
            category: 'backend',
            sourcePath: 'src/backend/payments/flat.ts',
            fission: {
                problem: 'Submit payment via Stripe',
                demand: ['PayInput'],
                answer: ['PayResult'],
                evidence: {
                    abstraction: {
        ...
        {
            nodeId: 'PaypalPay.submit',
            category: 'backend',
            sourcePath: 'src/backend/payments/flat.ts',
            fission: {
                problem: 'Submit payment via PayPal',
                demand: ['PayInput'],
                answer: ['PayResult'],
                evidence: {
                    abstraction: {
        ...
        {
            nodeId: 'WechatPay.submit',
            category: 'backend',
            sourcePath: 'src/backend/payments/flat.ts',
            fission: {
                problem: 'Submit payment via WeChat',
                demand: ['PayInput'],
                answer: ['PayResult'],
                evidence: {
                    abstraction: {
...
function createZeroHotspotOnlyTriadMap() {
    return [
        {
            nodeId: 'LegacyCalibrationPage.render',
            category: 'frontend',
            sourcePath: 'frontend/src/app/dashboard/workflows/designer/camera-calibration/page.tsx',
            fission: {
                problem: 'Render legacy calibration workflow page',
                demand: ['CalibrationState'],
                answer: ['CalibrationView'],
                evidence: {
                    abstraction: {
                        role: 'concrete',
                        signals: ['stateful_page'],
                        abstractionSignalCount: 0,
                        concreteSignalCount: 4,
                        concreteClassCount: 1,
                        publicMethodCount: 1,
                        topLevelExecutableCount: 2
                    }
                }
            }
        },
        {
            nodeId: 'CalibrationContract.describe',
            category: 'frontend',
            sourcePath: 'frontend/src/lib/calibration-contract.ts',
            fission: {
                problem: 'Describe calibration contracts',
                demand: ['CalibrationSchema'],
                answer: ['CalibrationContract'],
                evidence: {
                    abstraction: {
                        role: 'abstraction',
                        signals: ['contract'],
                        abstractionSignalCount: 9,
...
test('verify --json reports runtime edge consistency and governance metrics', () => {
    ...
    assert.equal(report.metrics.abstraction_profiled_source_count, 0);
...
test('verify skips abstraction governance checks when triad-map has no abstraction evidence coverage', () => {
    const root = createVerifyFixture([]);
    ...
    assert.equal(report.metrics.abstraction_profiled_source_count, 0);
    assert.equal(report.metrics.abstraction_deficit_index, 0);
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'abstraction_deficit_index' && check.status === 'skip'
        ),
        true
    );
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'variant_without_contract_cluster_count' && check.status === 'skip'
        ),
        true
    );
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'zero_abstraction_hotspot_count' && check.status === 'skip'
        ),
        true
    );
});

test('verify --strict fails when abstraction governance detects flat concrete-only growth', () => {
    const root = createVerifyFixture([], {
        triadMap: createAbstractionDeficitTriadMap()
    });

    const result = runCli(root, ['verify', '--strict', '--json']);
    assert.equal(result.status, 1, `verify --strict should fail on abstraction deficit governance` );
    const report = JSON.parse(result.stdout.slice(result.stdout.indexOf('{')));

    assert.equal(report.metrics.abstraction_profiled_source_count, 1);
    assert.equal(report.metrics.abstraction_signal_count, 0);
    assert.equal(report.metrics.concrete_signal_count, 6);
    assert.equal(report.metrics.flat_variant_cluster_count, 1);
    assert.equal(report.metrics.variant_without_contract_cluster_count, 1);
    assert.equal(report.metrics.abstraction_hotspot_count, 1);
    assert.equal(report.metrics.zero_abstraction_hotspot_count, 1);
    assert.equal(report.metrics.ghost_policy_violations, 0);
    assert.equal(report.metrics.abstraction_deficit_index >= 0.9, true);
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'abstraction_deficit_index' && check.status === 'fail'
        ),
        true
    );
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'variant_without_contract_cluster_count' && check.status === 'fail'
        ),
        true
    );
    assert.equal(
        report.checks.some(
            (check: { key?: string; status?: string }) =>
                check.key === 'zero_abstraction_hotspot_count' && check.status === 'fail'
        ),
        true
    );
});

test('verify baseline can classify zero abstraction hotspots as pre-existing debt', () => {
    const root = createVerifyFixture([], {
        triadMap: createZeroHotspotOnlyTriadMap()
    });
    fs.writeFileSync(
        path.join(root, '.triadmind', 'verify-baseline.json'),
        JSON.stringify(
            {
                generatedAt: new Date().toISOString(),
                runtime_unmatched_route_count: 0,
                zero_abstraction_hotspot_count: 1
            },
            null,
            2
        ),
    ...
    const result = runCli(root, ['verify', '--strict', '--json']);
    assert.equal(result.status, 0, `strict verify should pass when zero hotspots are within baseline: ${result.stderr || result.stdout}`);
    const report = JSON.parse(result.stdout.slice(result.stdout.indexOf('{')));
    const zeroHotspotCheck = report.checks.find(
        (check: { key?: string }) => check.key === 'zero_abstraction_hotspot_count'
    );

    assert.equal(zeroHotspotCheck.status, 'pass');
    assert.equal(zeroHotspotCheck.baseline.value, 1);
    assert.equal(zeroHotspotCheck.pre_existing, true);
    assert.equal(zeroHotspotCheck.introduced_by_current_diff, false);
    assert.equal(report.baseline.zero_abstraction_hotspot_count, 1);
});

test('verify --fail-on-new-regressions allows baselined pre-existing strict failures', () => {
    const root = createVerifyFixture([], {
        triadMap: createZeroHotspotOnlyTriadMap()
    });
    fs.writeFileSync(
    ...
    const result = runCli(root, ['verify', '--strict', '--json', '--fail-on-new-regressions']);
    assert.equal(result.status, 0, `new-regression mode should allow baselined pre-existing debt: ${result.stderr || result.stdout}`);
...
test('verify consumes retained ghost evidence when enforcing min-confidence policy', () => {
    const root = createVerifyFixture(
        [
            {
                level: 'info',
    ...
    const result = runCli(root, ['verify', '--strict', '--json']);
    assert.equal(result.status, 1, 'verify --strict should fail on low-confidence retained ghost evidence');
    const jsonStart = result.stdout.indexOf('{');
    assert.ok(jsonStart >= 0, 'verify --strict --json did not emit json payload');
    const report = JSON.parse(result.stdout.slice(jsonStart));
    assert.equal(report.metrics.ghost_policy_violations > 0, true);
    assert.equal(
        report.checks.some(
            (check: { key?: string; detail?: string; status?: string }) =>
                check.key === 'ghost_policy_compliance' &&
                check.status === 'fail' &&
                String(check.detail ?? '').includes('below minConfidence=')
        ),
        true
    );
});
