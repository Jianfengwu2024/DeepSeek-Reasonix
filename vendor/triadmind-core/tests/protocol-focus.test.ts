...
import { assertProtocolShape, readTriadMap } from '../protocol';
...
test('readTriadMap preserves fission evidence and passthrough node metadata', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'triadmind-read-triad-map-'));
    const mapPath = path.join(root, 'triad-map.json');
    fs.writeFileSync(
        mapPath,
        JSON.stringify(
            [
                {
                    ...
                    fission: {
                        ...
                        evidence: {
                            ghostReads: [
                                {
                                    raw: '[Ghost:Read] Cache (workflowCache)',
                                    mode: 'read',
                                    target: 'workflowCache',
                                    retainedInDemand: true,
                                    score: 5
                                }
                            ],
                            promotionReasons: ['runtime_signal'],
                            abstraction: {
                                role: 'mixed',
                                signals: ['strategy_family'],
                                variantCluster: 'workflow',
                                confidenceBand: 'p0-proof'
                            },
                            customTag: 'preserve-me'
                        }
                    },
                    topology: {
                        foldedLeaves: ['Workflow.build']
                    }
                }
            ],
            null,
    ...
    const nodes = readTriadMap(mapPath);
    assert.equal(nodes.length, 1);
    assert.equal(nodes[0].fission.evidence?.ghostReads?.[0]?.target, 'workflowCache');
    assert.deepEqual(nodes[0].fission.evidence?.promotionReasons, ['runtime_signal']);
    assert.equal(nodes[0].fission.evidence?.abstraction?.variantCluster, 'workflow');
    assert.equal((nodes[0].fission.evidence as Record<string, unknown>)?.customTag, 'preserve-me');
    assert.deepEqual((nodes[0] as { topology?: { foldedLeaves?: string[] } }).topology?.foldedLeaves, ['Workflow.build']);
});
...
test('assertProtocolShape preserves evidence on modify and create_child actions', () => {
    const existingNodes = createExistingNodes();
    const protocol = {
        protocolVersion: '1.0',
        project: 'focus-test',
        mapSource: '.triadmind/triad-map.json',
        userDemand: 'Preserve node evidence during protocol validation',
        actions: [
            {
                op: 'modify' as const,
                nodeId: 'Workflow.execute',
                ...
                fission: {
                    problem: 'Workflow orchestration',
                    demand: ['RunCommand', 'RoutingPolicy'],
                    answer: ['WorkflowResult'],
                    evidence: {
                        promotionReasons: ['business_semantic'],
                        abstraction: {
                            role: 'mixed',
                            signals: ['policy_port'],
                            variantCluster: 'workflow-routing'
                        },
                        customTag: 'modify-evidence'
                    }
                }
            },
            {
                op: 'create_child' as const,
                parentNodeId: 'Workflow.execute',
                node: {
                    nodeId: 'Workflow.routeByPolicy',
                    category: 'core',
                    sourcePath: 'src/workflow.ts',
                    fission: {
                        problem: 'Route workflow by policy',
                        demand: ['RunCommand', 'RoutingPolicy'],
                        answer: ['RoutePlan'],
                        evidence: {
                            abstraction: {
                                role: 'abstraction',
                                signals: ['strategy_family', 'policy_port'],
                                variantCluster: 'workflow-routing',
                                maturity: 'seed'
                            }
                        }
        ...
        resultTopology: [
            {
                ...
                topology: {
                    foldedLeaves: ['Workflow.buildRoutePlan']
                }
            }
        ]
    };

    const parsed = assertProtocolShape(protocol, { existingNodes });
    const modifyAction = parsed.actions.find(
        (action): action is Extract<(typeof parsed.actions)[number], { op: 'modify' }> => action.op === 'modify'
    );
    const createChildAction = parsed.actions.find(
        (action): action is Extract<(typeof parsed.actions)[number], { op: 'create_child' }> =>
            action.op === 'create_child'
    );

    assert.equal(modifyAction?.op, 'modify');
    assert.equal(modifyAction?.fission.evidence?.abstraction?.variantCluster, 'workflow-routing');
    assert.equal(
        (modifyAction?.fission.evidence as Record<string, unknown> | undefined)?.customTag,
        'modify-evidence'
    );

    assert.equal(createChildAction?.op, 'create_child');
    assert.equal(createChildAction?.node.fission.evidence?.abstraction?.role, 'abstraction');
    assert.equal(
        (createChildAction?.node.fission.evidence?.abstraction as Record<string, unknown> | undefined)?.maturity,
        'seed'
    );
    assert.deepEqual(
        (parsed.resultTopology?.[0] as { topology?: { foldedLeaves?: string[] } } | undefined)?.topology?.foldedLeaves,
        ['Workflow.buildRoutePlan']
    );
});
