...
import { readJsonArrayArtifactResult, readJsonObjectArtifactResult, readTriadNodesArtifact } from './artifactReaders';
...
export function listSnapshots(paths: WorkspacePaths) {
    const result = readJsonArrayArtifactResult<Pick<TriadSnapshot, 'id' | 'label' | 'createdAt'>>(paths.snapshotIndexFile);
    return result.value ?? ([] as Array<Pick<TriadSnapshot, 'id' | 'label' | 'createdAt'>>);
}
...
export function restoreSnapshot(paths: WorkspacePaths, snapshotId?: string) {
    ...
    const snapshotResult = readJsonObjectArtifactResult<TriadSnapshot>(snapshotPath);
...
function readNodeSourceMap(mapPath: string) {
    ...
    const nodes = readTriadNodesArtifact(mapPath);
    nodes.forEach((node) => {
