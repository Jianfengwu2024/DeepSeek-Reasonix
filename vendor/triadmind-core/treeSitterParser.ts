...
import JavaScript = require('triadmind-tree-sitter-javascript');
import Python = require('triadmind-tree-sitter-python');
import Go = require('triadmind-tree-sitter-go');
import Rust = require('triadmind-tree-sitter-rust');
import Cpp = require('triadmind-tree-sitter-cpp');
import Java = require('triadmind-tree-sitter-java');
import TypeScript = require('triadmind-tree-sitter-typescript');
import * as fs from 'fs';
import * as path from 'path';
import chalk from 'chalk';
import { readSourceFileText } from './artifactReaders';
import {
...
import { isRootLevelSupportAbstractionSourcePath, isSupportAbstractionSourcePath } from './supportAbstraction';
import { extractBindingNames as extractTreeSitterBindingNames } from './treeSitterBindingSupport';
import { scanTreeSitterGhostReferences, TreeSitterGhostAccessMode } from './treeSitterGhostScanner';
import { getPythonFunctionDefinitions, unwrapPythonDefinition } from './treeSitterPythonSupport';
import { createTreeSitterScanPlan } from './treeSitterScanPlanSupport';
...
export function collectTreeSitterParseResult(
    ...
    const scanPlan = createTreeSitterScanPlan(config.parser);
    ...
    const leafConfig: TriadConfig = {
        ...config,
        parser: scanPlan.leafParserConfig
    };
    const architectureConfig: TriadConfig = {
        ...config,
    ...
    for (const filePath of files) {
        let source: string;
        try {
            source = readSourceFileText(filePath);
        } catch (error: any) {
    ...
    for (const child of rootNode.namedChildren) {
        ...
        if (declarationNode.type === 'lexical_declaration' || declarationNode.type === 'variable_declaration') {
            for (const declarator of declarationNode.namedChildren.filter((node) => node.type === 'variable_declarator')) {
                ...
                const localName = extractTreeSitterBindingNames(nameNode, TREE_SITTER_PARSER_BINDING_PROFILE)[0];
                if (!localName) {
                    continue;
                }
...
function collectJavaScriptImportedBindings(
    ...
    for (const requireCall of rootNode.descendantsOfType('call_expression')) {
        ...
        const localNames = extractTreeSitterBindingNames(nameNode, TREE_SITTER_PARSER_BINDING_PROFILE);
...
function collectTypeScriptCapabilityNodes(
    ...
    for (const classNode of rootNode.descendantsOfType('class_declaration')) {
        triadGraph.push(
            ...collectTypeScriptClassCapabilityNodes(
                classNode,
                source,
                sourcePath,
                category,
    ...
    const moduleAggregateRecords =
        promotableTopLevel.length > 0
            ? promotableTopLevel
    ...
    if (triadGraph.length === 0 && moduleAggregateRecords.length > 0) {
        triadGraph.push(
            createTriadNode(
                ...
                category,
                sourcePath,
                mergeCapabilityDemand(moduleAggregateRecords.map((record) => record.demand)),
                mergeCapabilityAnswer(moduleAggregateRecords.map((record) => record.answer)),
...
type TypeScriptConcreteClassProfile = {
    className: string;
    implements: string[];
...
type TypeScriptAbstractionContext = {
    interfaceNames: Set<string>;
    typeAliasNames: Set<string>;
    abstractClassNames: Set<string>;
    abstractTypeNames: Set<string>;
    abstractFunctions: string[];
    concreteClassProfiles: Map<string, TypeScriptConcreteClassProfile>;
...
function buildTypeScriptAbstractionContext(
    rootNode: Parser.SyntaxNode,
    topLevelRecords: TypeScriptExecutableRecord[]
): TypeScriptAbstractionContext {
    ...
    const classVariantClusters = detectTypeScriptClassVariantClusters(
        concreteClassNodes
            .map((node) => getNameText(node.childForFieldName('name')))
            .filter(Boolean)
    );

    const concreteClassProfiles = new Map<string, TypeScriptConcreteClassProfile>();
...
function buildTypeScriptAbstractionEvidence(
    context: TypeScriptAbstractionContext,
    options: {
        ownerName?: string;
        demand?: string[];
        answer?: string[];
    }
): TriadNodeEvidence['abstraction'] {
    ...
    const dependsOnAbstractions = resolveReferencedTypeNames(
        [...(options.demand ?? []), ...(options.answer ?? [])],
        context.abstractTypeNames
    );
...
function collectTypeScriptInterfaceFunctionContracts(interfaceNode: Parser.SyntaxNode) {
...
function collectTypeScriptTypeAliasFunctionContracts(typeAliasNode: Parser.SyntaxNode) {
    ...
    if (typeNode.type === 'object_type') {
        return dedupeStringEntries(
            typeNode.namedChildren
...
function collectTypeScriptAbstractClassFunctionContracts(classNode: Parser.SyntaxNode) {
    const className = getNameText(classNode.childForFieldName('name'));
    if (!className) {
        return [] as string[];
    }
    ...
    for (const child of bodyNode?.namedChildren ?? []) {
        if (child.type === 'abstract_method_signature') {
            ...
            const parameters = formatTypeScriptParameterSignature(child.childForFieldName('parameters'));
            const returnType = normalizeTypeAnnotationNode(
                child.childForFieldName('return_type') ??
...
function formatTypeScriptContractSignatureChild(child: Parser.SyntaxNode, ownerName: string) {
...
function formatTypeScriptFunctionTypeSignature(name: string, functionTypeNode: Parser.SyntaxNode) {
    const parameters = formatTypeScriptParameterSignature(functionTypeNode.childForFieldName('parameters'));
    const returnType = normalizeTypeAnnotationNode(
        functionTypeNode.childForFieldName('return_type') ??
...
function isTypeScriptContractAliasNode(node: Parser.SyntaxNode) {
...
function isTypeScriptContractLikeType(node: Parser.SyntaxNode): boolean {
...
function extractTypeScriptHeritageTypeNames(node: Parser.SyntaxNode) {
    return dedupeStringEntries(
        node.namedChildren
            .filter((child) =>
...
function detectTypeScriptClassVariantClusters(classNames: string[]) {
...
function splitSemanticNameTokens(value: string) {
    return String(value ?? '')
...
function isIgnoredVariantClusterToken(value: string) {
    return new Set(['base', 'abstract', 'core', 'default', 'main', 'service', 'manager', 'handler']).has(
        String(value ?? '').trim().toLowerCase()
    );
}

function resolveReferencedTypeNames(values: string[], candidates: Set<string>) {
...
function collectTypeScriptClassCapabilityNodes(
    ...
    config: TriadConfig,
    ghostContext: GhostBindingContext,
    abstractionContext: TypeScriptAbstractionContext
) {
    const className = getNameText(classNode.childForFieldName('name'));
    const classBody = classNode.childForFieldName('body');
...
function isRuntimeExtractorSourcePath(sourcePath: string) {
    const normalizedPath = normalizeCapabilitySourcePath(sourcePath);
...
function isCommandRegistrationSourcePath(sourcePath: string) {
    const normalizedPath = normalizeCapabilitySourcePath(sourcePath);
    const fileStem = getCapabilitySourceFileStem(sourcePath);
    return /(^|\/)(commands?|subcommands?)(\/|$)/.test(normalizedPath) || fileStem.endsWith('commands');
}

function isInfrastructureHelperSourcePath(sourcePath: string) {
    const fileStem = getCapabilitySourceFileStem(sourcePath);
    return INFRASTRUCTURE_SOURCE_HINTS.some((hint) => fileStem.includes(hint));
}

function isWorkflowMaintenanceSourcePath(sourcePath: string) {
    return /workflow(?:$|[^a-z0-9])/i.test(getCapabilitySourceFileStem(sourcePath));
}

function hasStrongSurfaceCapabilitySignal(name: string, sourcePath: string, record?: CapabilityCandidateRecord) {
...
function isSupportAbstractionCapabilityNoiseCandidate(
    name: string,
    sourcePath: string,
    record: CapabilityCandidateRecord | undefined
) {
    if (!isSupportAbstractionSourcePath(sourcePath)) {
        return false;
    }
...
function isRuntimeExtractorHelperCapability(name: string, sourcePath: string) {
...
function isAdministrativeCommandRegistrationCapability(name: string, sourcePath: string, record?: CapabilityCandidateRecord) {
    return isCommandRegistrationSourcePath(sourcePath) &&
        COMMAND_REGISTRATION_PREFIXES.some((prefix) => hasNamePrefix(name, prefix)) &&
        !hasCapabilityDecorator(record);
}

function isInfrastructureHelperCapability(name: string, sourcePath: string, record?: CapabilityCandidateRecord) {
    return isInfrastructureHelperSourcePath(sourcePath) &&
        INFRASTRUCTURE_HELPER_PREFIXES.some((prefix) => hasNamePrefix(name, prefix)) &&
        !hasCapabilityDecorator(record);
}

function isWorkflowArtifactMaintenanceCapability(
    name: string,
    sourcePath: string,
    record: CapabilityCandidateRecord | undefined
) {
...
function shouldCreateSupportModuleAggregate(sourcePath: string, topLevelRecords: { name: string }[]) {
    ...
    return isRootLevelSupportAbstractionSourcePath(sourcePath);
}

function resolveTopLevelModuleAggregateRecords<T extends { name: string }>(
    sourcePath: string,
    topLevelRecords: T[],
    promotableTopLevel: T[]
) {
...
function isStaticRightBranchSourcePath(sourcePath: string) {
    return /rightbranch\.[^.]+$/i.test(normalizePath(String(sourcePath ?? '')).toLowerCase());
}
...
function shouldPromoteCapabilityByScore(
    ...
    const supportSource = isSupportAbstractionSourcePath(sourcePath);
    const hasOperationalSignal =
        hasDecoratorSignal || hasWorkflowSignal || hasFrontendSignal || hasAgentSignal || hasCliSignal || isPrimary || isExecuteLike;
...
function hasTypedCrossModuleGhostSignal(target: string, valueType: string) {
...
function isSimpleGhostSymbol(value: string) {
    return /^[A-Za-z_$][A-Za-z0-9_$]*$/.test(String(value ?? '').trim());
}

function normalizeGhostSymbol(value: string) {
    return String(value ?? '')
        .trim()
        .replace(/^this\./i, '')
        .replace(/^self\./i, '')
...
function collectPythonModuleBindings(rootNode: Parser.SyntaxNode) {
    ...
    for (const child of rootNode.namedChildren) {
        ...
        if (child.type === 'expression_statement') {
            ...
            const localName = extractTreeSitterBindingNames(leftNode, TREE_SITTER_PARSER_BINDING_PROFILE)[0];
            if (!localName) {
                continue;
            }
...
function collectPythonClassPropertyTypes(
    ...
    for (const child of classBody.namedChildren) {
        if (child.type === 'expression_statement') {
            ...
            const leftNode = assignmentNode.childForFieldName('left') ?? assignmentNode.namedChildren[0] ?? null;
            const propertyName = extractTreeSitterBindingNames(leftNode, TREE_SITTER_PARSER_BINDING_PROFILE)[0];
            if (!propertyName) {
                continue;
            }
...
function lookupPythonExportedBindingInfo(rootNode: Parser.SyntaxNode, bindingName: string): BindingInfo | undefined {
    for (const child of rootNode.namedChildren) {
        ...
        if (child.type === 'expression_statement') {
            ...
            const leftNode = assignmentNode.childForFieldName('left') ?? assignmentNode.namedChildren[0] ?? null;
            const name = extractTreeSitterBindingNames(leftNode, TREE_SITTER_PARSER_BINDING_PROFILE)[0];
            if (name === bindingName) {
...
function createTriadNode(
    ...
    problem?: string,
    foldedLeaves: string[] = [],
    abstractionEvidence?: TriadNodeEvidence['abstraction']
): TriadNode {
    ...
    const evidence =
        governedContracts.ghostReads.length > 0 || promotionReasons.length > 0 || abstractionEvidence
            ? {
