...
import { INTERNAL_ARTIFACT_PATH_CONTRACT_TYPES } from './artifactPathContracts';
import { readJsonObjectArtifactResult } from './artifactReaders';
import { isRootLevelSupportAbstractionSourcePath } from './supportAbstraction';
import { TriadConfigPaths, normalizePath } from './workspace';
...
const DEFAULT_CONFIG: TriadConfig = {
    ...
    parser: {
        ...
        genericContractIgnoreList: [
            ...
            'list[str]',
            'list[any]',
            ...INTERNAL_ARTIFACT_PATH_CONTRACT_TYPES
        ],
...
export function ensureTriadConfig(paths: TriadConfigPaths, force = false) {
...
export function loadTriadConfig(paths: TriadConfigPaths): TriadConfig {
...
export function registerMatureStableArchitectureAnchor(
    paths: TriadConfigPaths,
    anchor: {
        nodeId?: string;
        sourcePath?: string;
    }
...
export function ensureTriadProfile(paths: TriadConfigPaths, force = false) {
    fs.mkdirSync(paths.triadDir, { recursive: true });
...
export function loadTriadProfile(paths: TriadConfigPaths): TriadProfile {
...
export function createSourcePathFilter(projectRoot: string, config: TriadConfig) {
    ...
    return (sourcePath: string) => {
        ...
        return isRootLevelSupportAbstractionSourcePath(normalizedPath);
    };
}
...
function loadMergedJsonArtifact<T extends object>(
    filePath: string,
    fallback: T,
    merge: (value: Partial<T>) => T
) {
    const result = readJsonObjectArtifactResult<Partial<T>>(filePath);
    ...
    return {
        ...
        navigator: {
            autoGenerateProtocol:
                value.navigator?.autoGenerateProtocol ?? DEFAULT_CONFIG.navigator.autoGenerateProtocol,
            ...
            maxOutputTokens: normalizePositiveInteger(
                value.navigator?.maxOutputTokens,
                DEFAULT_CONFIG.navigator.maxOutputTokens
            ),
            temperature: normalizeTemperature(value.navigator?.temperature, DEFAULT_CONFIG.navigator.temperature),
            maxRepairRounds: normalizeNonNegativeInteger(
                value.navigator?.maxRepairRounds,
                DEFAULT_CONFIG.navigator.maxRepairRounds
            ),
        ...
        abstractionMemory: {
            ...
            autoSyncOnPrompt:
                value.abstractionMemory?.autoSyncOnPrompt ?? DEFAULT_CONFIG.abstractionMemory.autoSyncOnPrompt,
            excludeMatureStableSources:
                value.abstractionMemory?.excludeMatureStableSources ??
                DEFAULT_CONFIG.abstractionMemory.excludeMatureStableSources,
            excludeSourcePaths: normalizeStringArray(value.abstractionMemory?.excludeSourcePaths).map((entry) =>
                normalizeStableSourcePath(entry)
            ),
            excludeSourcePathPatterns: normalizeStringArray(value.abstractionMemory?.excludeSourcePathPatterns),
            minAbstractionRatio: normalizeConfidence(
                value.abstractionMemory?.minAbstractionRatio,
                DEFAULT_CONFIG.abstractionMemory.minAbstractionRatio
            ),
            maxPromptEntries: normalizePositiveInteger(
                value.abstractionMemory?.maxPromptEntries,
                DEFAULT_CONFIG.abstractionMemory.maxPromptEntries
            ),
            maxSearchResults: normalizePositiveInteger(
                value.abstractionMemory?.maxSearchResults,
                DEFAULT_CONFIG.abstractionMemory.maxSearchResults
            )
        },
        topologyRisk: {
            ...
            matureStableSourcePaths: normalizeStringArray(value.topologyRisk?.matureStableSourcePaths).map((entry) =>
                normalizeStableSourcePath(entry)
            ),
...
function normalizeNavigatorLlmProvider(value: NavigatorLlmProvider | undefined): NavigatorLlmProvider {
...
function normalizeImpactTiers(value: Partial<TriadImpactTiersConfig> | undefined): TriadImpactTiersConfig {
    const shortChainMax = normalizeNonNegativeInteger(
        value?.shortChainMax,
        DEFAULT_CONFIG.impactTiers.shortChainMax
    );
    const rawMediumChainMax = normalizeNonNegativeInteger(
        value?.mediumChainMax,
        DEFAULT_CONFIG.impactTiers.mediumChainMax
    );
...
function normalizeForceTier(value: Record<string, TriadImpactTier> | undefined): Record<string, TriadImpactTier> {
    if (!value || typeof value !== 'object') {
        return {};
    }
    const result: Record<string, TriadImpactTier> = {};
...
function normalizeNonEmptyText(value: string | undefined, fallback: string) {
    const normalized = String(value ?? '').trim();
    return normalized || fallback;
}

function normalizeStableSourcePath(value: string | undefined) {
...
function mergeExactStableNodeIds(existing: string[], candidate: string) {
    return candidate ? Array.from(new Set([...existing, candidate])) : [...existing];
}

function mergeExactStableSourcePaths(existing: string[], candidate: string) {
    return candidate ? Array.from(new Set([...existing, candidate])) : [...existing];
}
...
function defaultScanCategories(categories: TriadCategoryMap) {
...
function normalizeCategoryFromScope(scope: TriadScanScope | undefined) {
    const category = String(scope?.category ?? '').trim();
    return category || undefined;
}

function resolveSelectedScanScopes(config: TriadConfig) {
...
function matchesAnyScanScope(normalizedPath: string, scopes: TriadScanScope[]) {
    if (scopes.length === 0) {
        return false;
    }
