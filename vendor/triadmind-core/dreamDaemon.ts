...
function readDaemonPid(filePath: string) {
    const result = readJsonObjectArtifactResult<Partial<DreamDaemonPidRecord>>(filePath);
...
function readDreamDaemonState(filePath: string): DreamDaemonState {
    const result = readJsonObjectArtifactResult<Partial<DreamDaemonState>>(filePath);
    if (result.status !== 'ok' || !result.value) {
        return {
