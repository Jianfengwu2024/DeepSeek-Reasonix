...
function readDreamAutoState(filePath: string): DreamAutoState {
    const result = readJsonObjectArtifactResult<Partial<DreamAutoState>>(filePath);
...
function readDreamLock(lockFilePath: string) {
    const result = readJsonObjectArtifactResult<Partial<DreamLockPayload>>(lockFilePath);
    if (result.status !== 'ok' || !result.value) {
        return undefined;
    }
