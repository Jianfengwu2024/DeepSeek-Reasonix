...
import { readSourceFileText, readTextIfExists, readTriadNodesArtifact } from './artifactReaders';
import {
...
function runPolyglotParser(language: PolyglotLanguage, projectRoot: string, outputPath?: string, configOverride?: TriadConfig): void {
    ...
    walkProject(projectRoot, (absolutePath) => {
        ...
        let content: string;
        try {
            content = readSourceFileText(absolutePath);
        } catch (error: any) {
...
function loadNodeLocations(projectRoot: string) {
    ...
    for (const candidate of candidates) {
        ...
        const nodes = readTriadNodesArtifact(candidate);
